...
2. 浏览器打开本应用 → **选择 ZIP 上传**（推荐整包；勿只传解压后的巨型 XML）  
3. 大导出（包内 XML 常 400–600MB+）在 **v2.5.21+** 走字节流式解析；进度可见「流式解析 %」  
4. 在 **分析概览** 查看多域 KPI（不应只有心电）；可选填写个人背景（用药/关注点，仅本机）  
5. 点 **复制完整提示词**（概览按钮或底部吸底栏）  
6. 粘贴到豆包 / Kimi / ChatGPT 等 → 得到结构化分析报告  

大包说明与故障排查：[`docs/REAL_DEVICE_ZIP.md`](docs/REAL_DEVICE_ZIP.md) · 数据恢复：[`docs/DATA_RECOVERY.md`](docs/DATA_RECOVERY.md)
...
- ✅ **大 ZIP 流式导入（v2.5.21+）**：`parseHealthXmlAsync` 按字节解析，规避 ~512MB 字符串上限；解压跳过 CDA/轨迹  
- ✅ 中小 XML 仍可 Worker / 主线程解析（失败回退）  
- ✅ 深色模式（跟随系统或手动切换）  
- ✅ 结果概览 KPI + 吸底「复制完整提示词」  
...
| **v2.5.10** | **桌面健康指挥台**：总览三层（状态主卡 → 7/30 日 sparkline 趋势带 → KPI+信号）；无 ECharts 首屏成本 |
| **v2.5.11** | **趋势/报告/数据页产品化**：克制 stagger 入场；去工程词导语；报告舞台与趋势控件区视觉对齐 |
| **v2.5.12** | **数据页分组叙事 + 生理色 token**：状态/导出/备份/空间/隐私五段；恢复·睡眠·代谢·活动分区色 |
| **v2.5.13** | **壳层与叙事串联**：顶栏日期+数据新鲜度+设置；侧栏用户向描述；优先关注 → 依据与线索 视觉连线 |
| **v2.5.14** | **空态/加载态产品化 + 趋势域 pill**：空态图标与步骤；加载呼吸点；指标切换滑动选中条 |
| **v2.5.15** | **报告类型 pill + 导入成功条**：报告 kind 滑动选中；总览 loading→ready 轻量成功提示（自动消失） |
| **v2.5.16** | **范围 pill + 成功条 CTA**：趋势/总览 7·30·90 日 pill；成功条「打开趋势」次要按钮 |
| **v2.5.17** | **对比 pill + 手机成功条**：趋势叠加对比改为 pill；手机成功条吸顶、隐藏详情行 |
| **v2.5.18** | **图表「我的视图」+ 暗色分区色**：预设区产品化；暗色域色略降饱和 |
| **v2.5.19** | **报告类型记忆 + 安全区**：本机记住上次报告 kind；手机成功条/主区避让底栏 |
| **v2.5.20** | **信息去重 + 卡片分级 + 大屏焦点**：快照仅区间/新鲜度/异常；信号默认 3 条可展开；主次卡片视觉级；TV 非焦点 ≥0.75 |
| **v2.5.21** | **大 ZIP 流式导入**：导出.xml >512MB 不再整串 decode（引擎字符串上限）；按字节解析并跳过 CDA/轨迹；文档见 [`docs/REAL_DEVICE_ZIP.md`](docs/REAL_DEVICE_ZIP.md) |
