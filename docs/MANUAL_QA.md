...
**版本：** v1.85+（年分片 keep-N、双域一键裁剪、保存后可选自动裁剪、**睡眠/步数年分片**）  
...
- [ ] **复制仓状态摘要**：复制内容仅为 meta（占用、分片月份/年份列表含 sleep/steps 年若有、授权与保留偏好等），**不含** CGM/血压/体重/睡眠日值/步数明细等原始时序
