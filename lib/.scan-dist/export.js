...
exports.generateVisitSummaryMarkdown = exports.generateWeeklyReportMarkdown = void 0;
exports.buildExportBundle = buildExportBundle;
exports.joinCsvBundle = joinCsvBundle;
const snapshot_1 = require("./snapshot");
const signals_1 = require("./signals");
var weekly_report_1 = require("./weekly-report");
Object.defineProperty(exports, "generateWeeklyReportMarkdown", { enumerable: true, get: function () { return weekly_report_1.generateWeeklyReportMarkdown; } });
var visit_summary_1 = require("./visit-summary");
Object.defineProperty(exports, "generateVisitSummaryMarkdown", { enumerable: true, get: function () { return visit_summary_1.generateVisitSummaryMarkdown; } });
...
function buildExportBundle(analysis) {
    const snapshot = (0, snapshot_1.buildAnalysisSnapshot)(analysis);
    const signals = (0, signals_1.detectCrossSignals)(analysis);
    ...
    if (analysis.cgmStats) {
        ...
        csvFiles.push({
            ...
            content: toCsv(['date', 'count', 'mean', 'min', 'max', 'cv', 'pctBelow39', 'pctAbove78', 'pctAbove100'], Object.keys(daily)
                .sort()
                .map((d) => {
                const x = daily[d];
    ...
    if (data.bloodPressure.length) {
        csvFiles.push({
            ...
            content: toCsv(['datetime', 'date', 'systolic', 'diastolic'], data.bloodPressure.map((r) => [r.datetime, r.date, r.systolic, r.diastolic])),
        });
    }
    if (data.weight.length) {
        csvFiles.push({
            filename: 'weight.csv',
            content: toCsv(['datetime', 'date', 'value_kg', 'body_fat_pct'], data.weight.map((w) => [w.datetime, w.date, w.value, w.bodyFat ?? ''])),
        });
    }
    if (analysis.weightStats?.trendSeries?.length) {
        csvFiles.push({
            filename: 'weight_trend_daily.csv',
            content: toCsv(['date', 'trend_kg', 'body_fat_pct', 'morning_kg', 'evening_kg', 'raw_count'], analysis.weightStats.daily.map((d) => [
                d.date,
                d.trend.value,
                d.trend.bodyFat ?? '',
                d.morning?.value ?? '',
    ...
    if (data.bodyFat?.length) {
        csvFiles.push({
            ...
            content: toCsv(['datetime', 'date', 'body_fat_pct', 'source'], data.bodyFat.map((f) => [f.datetime, f.date, f.value, f.source ?? ''])),
        });
    }
    ...
    if (hrvDates.length) {
        csvFiles.push({
            ...
            content: toCsv(['date', 'allMean', 'overnightMean', 'min', 'max', 'count'], hrvDates.map((d) => {
    ...
    if (hrDates.length) {
        csvFiles.push({
            ...
            content: toCsv(['date', 'resting', 'walking'], hrDates.map((d) => [d, rest[d] ?? '', walk[d] ?? ''])),
        });
    }
    ...
    if (stepDates.length) {
        csvFiles.push({
            filename: 'steps.csv',
            content: toCsv(['date', 'steps'], stepDates.map((d) => [d, steps[d]])),
        });
    }
    ...
    if (sleepDates.length) {
        csvFiles.push({
            filename: 'sleep.csv',
            content: toCsv(['date', 'total_h', 'deep_h', 'rem_h', 'core_h', 'awake_h'], sleepDates.map((d) => {
                const s = sleep[d];
    ...
    if (signals.length) {
        csvFiles.push({
            ...
            content: toCsv(['severity', 'date', 'title', 'detail', 'dimensions'], signals.map((s) => [s.severity, s.date || '', s.title, s.detail, s.dimensions.join('|')])),
        });
    }
