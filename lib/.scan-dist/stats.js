...
exports.normalizeRecoveryWeights = normalizeRecoveryWeights;
exports.calcCgmStats = calcCgmStats;
exports.calcBloodPressureStats = calcBloodPressureStats;
exports.calcWeightStats = calcWeightStats;
exports.summarizeHrvByDay = summarizeHrvByDay;
exports.calcWatchStats = calcWatchStats;
exports.calcWorkoutStats = calcWorkoutStats;
exports.enrichEcgWithContext = enrichEcgWithContext;
exports.calcEcgStats = calcEcgStats;
exports.attachRecoveryBaseline = attachRecoveryBaseline;
exports.calcRecoveryWeek = calcRecoveryWeek;
exports.calcRecoveryWeeks = calcRecoveryWeeks;
exports.recomputeRecovery = recomputeRecovery;
...
function normalizeRecoveryWeights(weights) {
    const base = { ...types_1.DEFAULT_RECOVERY_WEIGHTS };
    if (!weights)
        return base;
...
function weightedMean(parts) {
    ...
    for (const p of parts) {
        if (!Number.isFinite(p.value) || !Number.isFinite(p.weight) || p.weight <= 0)
            continue;
...
function calcCgmStats(cgm) {
    ...
    const firstDayDate = (0, parser_1.getDate)(sorted[0].datetime);
    const firstDayPoints = sorted.filter((p) => (0, parser_1.getDate)(p.datetime) === firstDayDate);
    const stablePoints = sorted.filter((p) => (0, parser_1.getDate)(p.datetime) !== firstDayDate);
    const firstDay = cgmSegment(firstDayPoints);
    const stable = stablePoints.length ? cgmSegment(stablePoints) : null;
    // 分日统计
    const byDay = {};
    for (const p of sorted) {
        const d = (0, parser_1.getDate)(p.datetime);
        if (!byDay[d])
            byDay[d] = [];
    ...
    for (const window of [30, 60, 120]) {
        ...
        for (let right = 0; right < sorted.length; right++) {
            const rightMs = (0, parser_1.parseAppleDate)(sorted[right].datetime);
            while (left < right && rightMs - (0, parser_1.parseAppleDate)(sorted[left].datetime) > windowMs) {
                left++;
            }
...
function calcBloodPressureStats(records) {
    ...
    const isMorning = (r) => (0, parser_1.getHour)(r.datetime) < 12;
    const isEvening = (r) => (0, parser_1.getHour)(r.datetime) >= 18;
    return {
        records: sorted,
...
function calcWeightStats(weight) {
    ...
    for (const date of Object.keys(byDate).sort()) {
        ...
        const mornings = all.filter((w) => (0, parser_1.getHour)(w.datetime) < 12);
        const evenings = all.filter((w) => (0, parser_1.getHour)(w.datetime) >= 18);
...
function summarizeHrvByDay(hrv, hrvOvernight) {
    ...
    for (const date of Object.keys(hrv).sort()) {
        ...
        result[date] = {
            ...
            overnightMean: overnight.length > 0
...
function calcWatchStats(watchDaily) {
    if (!watchDaily || !Object.keys(watchDaily).length)
        return null;
    const days = Object.keys(watchDaily)
        .sort()
    ...
    return {
        ...
        breathingDisturbanceMean7d: meanLastN(days.map((d) => d.breathingDisturbance), 7),
        breathingDisturbanceLatest,
        daylightMinMean7d: meanLastN(days.map((d) => (d.daylightMin > 0 ? d.daylightMin : null)), 7),
        standHoursMean7d: meanLastN(days.map((d) => (d.standHoursStood > 0 || d.standHoursIdle > 0 ? d.standHoursStood : null)), 7),
...
function calcWorkoutStats(workouts, referenceDate) {
    ...
    const latestDate = referenceDate && /^\d{4}-\d{2}-\d{2}$/.test(referenceDate)
        ? referenceDate
        : last.date;
    ...
    for (const s of sessions) {
        const cur = byTypeMap.get(s.activityType) || {
            ...
            activityLabel: s.activityLabel || (0, parser_1.workoutTypeLabel)(s.activityType),
            count: 0,
            durationMin: 0,
            activeKcal: 0,
        };
...
function enrichEcgWithContext(ecg, workouts, activity) {
    ...
    const workoutStarts = (workouts || [])
        .map((w) => (0, parser_1.parseAppleDate)(w.startDate))
        .filter((t) => Number.isFinite(t))
        .sort((a, b) => a - b);
    const nearWorkout = (t) => {
        if (!Number.isFinite(t) || !workoutStarts.length)
            return false;
        // 线性扫描即可（ECG / workout 规模通常很小）；有序可提前结束
        for (const ws of workoutStarts) {
            const d = Math.abs(ws - t);
            if (d <= ECG_NEAR_WORKOUT_MS)
                return true;
            if (ws > t + ECG_NEAR_WORKOUT_MS)
                break;
        }
        return false;
    };
    ...
    for (const e of highHrs) {
        const hour = (0, parser_1.getHour)(e.datetime);
        if (Number.isFinite(hour) && hour >= 0 && hour <= 23) {
            highHrByHour[hour] += 1;
        }
        const t = (0, parser_1.parseAppleDate)(e.datetime);
        const near = nearWorkout(t);
        if (near)
            highHrNearWorkoutCount += 1;
        // 非运动窗：夜间/清晨 22–08，或附近无 Workout
        if (isNightOrEarlyHour(hour) || !near) {
            highHrRestingWindowCount += 1;
        }
        const day = (0, parser_1.getDate)(e.datetime);
...
function calcEcgStats(ecg, workouts, activity) {
    ...
    for (const e of sorted) {
        ...
        if (/窦性|Sinus/i.test(c))
            sinusCount += 1;
        else if (isHighHrClassification(c))
            highHrCount += 1;
        else if (/不佳|Inconclusive|Poor/i.test(c))
            inconclusiveCount += 1;
        else
            otherCount += 1;
    }
...
function workoutWindowAt(sessions, endDate, windowDays) {
    ...
    const list = sessions.filter((s) => s.date <= endDate && daysBetween(s.date, endDate) <= windowDays - 1);
    return {
        count: list.length,
...
function attachRecoveryBaseline(week, recoveryWeeks, localeInput) {
    const L = (0, locale_1.createL)((0, locale_1.normalizeLocale)(localeInput));
    ...
    if (week.recoveryScore != null && priorScores.length >= 4) {
        ...
        if (med != null) {
            ...
            if (Math.abs(vsBaselineDelta) >= 8) {
                ...
                statusLabel =
                    vsBaselineDelta > 0
                        ? L(`${statusLabel}（高于近几周中位约 ${abs} 分）`, `${statusLabel} (~${abs} pts above recent median)`)
                        : L(`${statusLabel}（低于近几周中位约 ${abs} 分）`, `${statusLabel} (~${abs} pts below recent median)`);
            }
        }
    }
    return {
...
function scoreRecoveryLoad(input) {
    const L = (0, locale_1.createL)((0, locale_1.normalizeLocale)(input.locale));
...
function buildRecoveryWeekAt(analysis, weekEnd, weights, locale) {
    ...
    const standHoursMean7d = meanWatchSeriesLastN(days, (d) => (d.standHoursStood > 0 || d.standHoursIdle > 0 ? d.standHoursStood : null), 7, weekEnd);
    const daylightMinMean7d = meanWatchSeriesLastN(days, (d) => (d.daylightMin > 0 ? d.daylightMin : null), 7, weekEnd);
...
function calcRecoveryWeek(analysis, options) {
    ...
    if (!end)
        return null;
    const week = buildRecoveryWeekAt(analysis, end, options?.recoveryWeights, options?.locale);
    if (!week)
        return null;
    if (options?.skipBaseline)
        return week;
    const weeks = options?.recoveryWeeks !== undefined
...
function analyzeAll(data, options) {
    const allDates = [
        ...data.cgm.map((x) => (0, parser_1.getDate)(x.datetime)),
        ...
        ...(data.ecg || []).map((e) => (0, parser_1.getDate)(e.datetime)),
    ];
    allDates.sort();
    const start = allDates[0] || '';
    const end = allDates[allDates.length - 1] || '';
    const hrvByDate = summarizeHrvByDay(data.hrv, data.hrvOvernight);
    const stepsByDate = Object.fromEntries(Object.entries(data.steps).map(([d, v]) => [d, v.max]));
