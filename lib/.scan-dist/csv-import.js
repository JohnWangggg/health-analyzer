...
exports.parseWeightScaleCsv = parseWeightScaleCsv;
exports.parseBloodPressureCsv = parseBloodPressureCsv;
exports.mergeExternalCsvIntoData = mergeExternalCsvIntoData;
...
function parseWeightScaleCsv(text) {
    ...
    for (let r = 1; r < lines.length; r++) {
        ...
        const rec = {
            datetime,
            date: (0, parser_1.getDate)(datetime),
            value,
        };
        if (iFat >= 0) {
            const fat = parseNum(cols[iFat]);
            if (fat != null && fat > 0 && fat < 80)
                rec.bodyFat = fat;
        }
        if (iBmi >= 0) {
...
function mergeExternalCsvIntoData(data, options = {}) {
    ...
    if (options.weightCsvText) {
        ...
        for (const row of rows) {
            const hit = data.weight.find((w) => sameMinute(w.datetime, row.datetime) || (w.date === row.date && Math.abs(w.value - row.value) < 0.05));
            if (hit) {
                let updated = false;
    ...
    if (options.bpCsvText) {
        ...
        for (const row of rows) {
            const hit = data.bloodPressure.find((b) => sameMinute(b.datetime, row.datetime) ||
