...
exports.SHORT_SYSTEM_PROMPT_EN = exports.SHORT_SYSTEM_PROMPT = exports.MAIN_PROMPT_TEMPLATE_EN = exports.MAIN_PROMPT_TEMPLATE = void 0;
exports.formatUserContext = formatUserContext;
exports.formatAnalysisForLLM = formatAnalysisForLLM;
exports.generateLLMPrompt = generateLLMPrompt;
exports.generateDataOnly = generateDataOnly;
const signals_1 = require("../signals");
const insights_1 = require("../insights");
const zh_tw_map_1 = require("../zh-tw-map");
const locale_1 = require("../locale");
/** 主提示词：引导 LLM 按指定格式输出深度分析报告（中文） */
exports.MAIN_PROMPT_TEMPLATE = `# 角色与任务
...
exports.MAIN_PROMPT_TEMPLATE_EN = `# Role & Task
...
function hasAnyUserContext(ctx) {
    if (!ctx)
        return false;
    return Boolean((ctx.age != null && Number.isFinite(Number(ctx.age))) ||
        trimText(ctx.sex) ||
...
function formatUserContext(ctx, options) {
    if (!hasAnyUserContext(ctx) || !ctx)
        return '';
    const L = (0, locale_1.createL)(options?.locale);
    const lines = [
        L('## 个人背景（用户自述，仅供对照，非医疗档案）', '## Personal background (user-reported, for context only — not a medical record)'),
        '',
        L('| 项目 | 内容 |', '| Item | Value |'),
        '|---|---|',
    ];
    if (ctx.age != null && Number.isFinite(Number(ctx.age))) {
        lines.push(L(`| 年龄 | ${Number(ctx.age)} 岁 |`, `| Age | ${Number(ctx.age)} years |`));
    }
    if (trimText(ctx.sex)) {
        lines.push(L(`| 性别 | ${trimText(ctx.sex)} |`, `| Sex | ${trimText(ctx.sex)} |`));
    }
    if (ctx.heightCm != null && Number.isFinite(Number(ctx.heightCm))) {
        lines.push(L(`| 身高 | ${Number(ctx.heightCm)} cm |`, `| Height | ${Number(ctx.heightCm)} cm |`));
    }
    if (ctx.targetWeightKg != null && Number.isFinite(Number(ctx.targetWeightKg))) {
        lines.push(L(`| 目标体重 | ${Number(ctx.targetWeightKg)} kg |`, `| Target weight | ${Number(ctx.targetWeightKg)} kg |`));
    }
    if (trimText(ctx.medications)) {
        lines.push(L(`| 当前用药 | ${trimText(ctx.medications)} |`, `| Current medications | ${trimText(ctx.medications)} |`));
    }
    if (trimText(ctx.conditions)) {
        lines.push(L(`| 已知情况 | ${trimText(ctx.conditions)} |`, `| Known conditions | ${trimText(ctx.conditions)} |`));
    }
    if (trimText(ctx.focus)) {
        lines.push(L(`| 本次关注点 | ${trimText(ctx.focus)} |`, `| Focus this time | ${trimText(ctx.focus)} |`));
    }
    if (trimText(ctx.notes)) {
        lines.push(L(`| 补充说明 | ${trimText(ctx.notes)} |`, `| Notes | ${trimText(ctx.notes)} |`));
    }
    lines.push('');
    lines.push(L('> 以上为用户本地填写的自述信息，可能不完整；解读时作背景参考，不得据此开药或下诊断。', '> The above is local user-reported information and may be incomplete; use only as background. Do not prescribe or diagnose from it.'));
    lines.push('');
    return lines.join('\n');
}
/**
...
function formatAnalysisForLLM(analysis, options) {
    const L = (0, locale_1.createL)(options?.locale);
    const sections = [];
    const { data, cgmStats, bpStats, weightStats, watchStats, workoutStats, ecgStats, recoveryWeek, recoveryWeeks, hrvByDate, dateRange, } = analysis;
    const detailDays = 90;
    ...
    const fmtSeg = (title, o) => {
        sections.push(L(`**${title}**（共 ${o.count} 条，${o.timeRange}）`, `**${title}** (${o.count} records, ${o.timeRange})`));
        sections.push(``);
    ...
    sections.push(L(`> 明细表默认展示最近 ${detailDays} 天；更早数据已纳入总体统计，但为控制提示词长度未逐条展开。`, `> Detail tables default to the last ${detailDays} days; earlier data is included in overall stats but not expanded row-by-row to limit prompt size.`));
    sections.push(L(`> 体重趋势默认取**每日晨起**（12:00 前最早一条，若无则取全日最早）；CGM 请优先看**稳定期**（排除传感器首个日历日）。`, `> Weight trend defaults to **morning** (earliest reading before 12:00, else earliest of the day); for CGM prefer the **stable period** (exclude sensor first calendar day).`));
    sections.push(``);
    // Data availability
    const av = data.dataAvailability;
    sections.push(L(`## 数据可用性`, `## Data availability`));
    sections.push(``);
    sections.push(L(`| 维度 | 是否存在 | 数据量 |`, `| Dimension | Available | Volume |`));
    sections.push(`|---|---|---|`);
    sections.push(L(`| CGM 动态血糖 | ${av.hasCgm ? '✅' : '❌'} | ${unitRecords(data.cgm.length)} |`, `| CGM continuous glucose | ${av.hasCgm ? '✅' : '❌'} | ${unitRecords(data.cgm.length)} |`));
    sections.push(L(`| 血压 | ${av.hasBloodPressure ? '✅' : '❌'} | ${unitRecords(data.bloodPressure.length)} |`, `| Blood pressure | ${av.hasBloodPressure ? '✅' : '❌'} | ${unitRecords(data.bloodPressure.length)} |`));
    sections.push(L(`| 体重 | ${av.hasWeight ? '✅' : '❌'} | ${unitRecords(data.weight.length)} 原始 / ${weightStats?.dayCount ?? 0} 趋势日 |`, `| Weight | ${av.hasWeight ? '✅' : '❌'} | ${unitRecords(data.weight.length)} raw / ${weightStats?.dayCount ?? 0} trend days |`));
    sections.push(L(`| 体脂 | ${av.hasBodyFat ? '✅' : '❌'} | ${unitRecords(data.bodyFat?.length ?? 0)} / ${weightStats?.bodyFatDayCount ?? 0} 趋势日 |`, `| Body fat | ${av.hasBodyFat ? '✅' : '❌'} | ${unitRecords(data.bodyFat?.length ?? 0)} / ${weightStats?.bodyFatDayCount ?? 0} trend days |`));
    sections.push(L(`| HRV | ${av.hasHrv ? '✅' : '❌'} | ${unitDays(Object.keys(hrvByDate).length)} |`, `| HRV | ${av.hasHrv ? '✅' : '❌'} | ${unitDays(Object.keys(hrvByDate).length)} |`));
    sections.push(L(`| 静息/步行心率 | ${av.hasHeartRate ? '✅' : '❌'} | ${unitDays(Object.keys(data.restingHr).length)} |`, `| Resting / walking HR | ${av.hasHeartRate ? '✅' : '❌'} | ${unitDays(Object.keys(data.restingHr).length)} |`));
    sections.push(L(`| 步数 | ${av.hasSteps ? '✅' : '❌'} | ${unitDays(Object.keys(data.steps).length)} |`, `| Steps | ${av.hasSteps ? '✅' : '❌'} | ${unitDays(Object.keys(data.steps).length)} |`));
    sections.push(L(`| 睡眠 | ${av.hasSleep ? '✅' : '❌'} | ${unitDays(Object.keys(data.sleep).length)} |`, `| Sleep | ${av.hasSleep ? '✅' : '❌'} | ${unitDays(Object.keys(data.sleep).length)} |`));
    sections.push(L(`| Watch 活动 | ${av.hasWatchActivity ? '✅' : '❌'} | ${unitDays(watchStats?.dayCount ?? Object.keys(data.watchDaily || {}).length)} |`, `| Watch activity | ${av.hasWatchActivity ? '✅' : '❌'} | ${unitDays(watchStats?.dayCount ?? Object.keys(data.watchDaily || {}).length)} |`));
    sections.push(L(`| 血氧 SpO₂ | ${av.hasSpO2 ? '✅' : '❌'} | ${watchStats?.spo2DayCount ?? 0} 天有样本 |`, `| SpO₂ | ${av.hasSpO2 ? '✅' : '❌'} | ${watchStats?.spo2DayCount ?? 0} days with samples |`));
    sections.push(L(`| 呼吸频率 | ${av.hasRespiratoryRate ? '✅' : '❌'} | — |`, `| Respiratory rate | ${av.hasRespiratoryRate ? '✅' : '❌'} | — |`));
    sections.push(L(`| VO₂ max | ${av.hasVo2Max ? '✅' : '❌'} | ${unitDays(watchStats?.vo2DayCount ?? 0)} |`, `| VO₂ max | ${av.hasVo2Max ? '✅' : '❌'} | ${unitDays(watchStats?.vo2DayCount ?? 0)} |`));
    sections.push(L(`| 睡眠腕温 | ${av.hasWristTemp ? '✅' : '❌'} | — |`, `| Sleep wrist temp | ${av.hasWristTemp ? '✅' : '❌'} | — |`));
    sections.push(L(`| 睡眠呼吸紊乱 | ${(watchStats?.breathingDisturbanceDayCount ?? 0) > 0 ? '✅' : '❌'} | ${unitDays(watchStats?.breathingDisturbanceDayCount ?? 0)} |`, `| Sleep breathing disturbances | ${(watchStats?.breathingDisturbanceDayCount ?? 0) > 0 ? '✅' : '❌'} | ${unitDays(watchStats?.breathingDisturbanceDayCount ?? 0)} |`));
    sections.push(L(`| Workout 会话 | ${av.hasWorkouts ? '✅' : '❌'} | ${unitSessions(workoutStats?.count ?? data.workouts?.length ?? 0)} |`, `| Workout sessions | ${av.hasWorkouts ? '✅' : '❌'} | ${unitSessions(workoutStats?.count ?? data.workouts?.length ?? 0)} |`));
    sections.push(L(`| ECG | ${av.hasEcg ? '✅' : '❌'} | ${unitReports(data.ecg.length)} |`, `| ECG | ${av.hasEcg ? '✅' : '❌'} | ${unitReports(data.ecg.length)} |`));
    sections.push(``);
    sections.push(L(`数据时间范围：${dateRange.start} 至 ${dateRange.end}`, `Data date range: ${dateRange.start} to ${dateRange.end}`));
    const dq = data.dataQuality;
    if (dq && dq.skippedFutureCount > 0) {
        sections.push(``);
        sections.push(L(`### 数据质量提示（未来日期已排除）`, `### Data quality note (future-dated records excluded)`));
        sections.push(``);
        sections.push(L(`- 参考日（本地「今天」）：\`${dq.referenceDate}\``, `- Reference day (local “today”): \`${dq.referenceDate}\``));
        sections.push(L(`- 已跳过 **${dq.skippedFutureCount}** 条起始日期晚于参考日的记录（常见于误录的未来体重等）`, `- Skipped **${dq.skippedFutureCount}** records whose start date is after the reference day (often mis-entered future weights, etc.)`));
        if (dq.futureSampleDates && dq.futureSampleDates.length) {
            sections.push(L(`- 见到的未来日期样本：${dq.futureSampleDates.map((d) => `\`${d}\``).join('、')}`, `- Future date samples seen: ${dq.futureSampleDates.map((d) => `\`${d}\``).join(', ')}`));
        }
        sections.push(L(`- 请在 iPhone「健康」App 中核对并删除错误未来条目；本报告统计**不包含**这些未来记录`, `- Please review and delete erroneous future entries in the iPhone Health app; this report’s stats **exclude** those future records`));
    }
    sections.push(``);
    // CGM
    if (cgmStats) {
        ...
        if (cgmStats.firstDayDate) {
            sections.push(L(`> 传感器首个日历日为 \`${cgmStats.firstDayDate}\`，该日低值易为佩戴/校准伪影；**解读请优先采用稳定期**。`, `> Sensor first calendar day is \`${cgmStats.firstDayDate}\`; lows that day are often wear/calibration artifacts; **prefer the stable period for interpretation**.`));
            sections.push(``);
        }
        fmtSeg(L('全程统计', 'Overall stats'), cgmStats.overall);
        if (cgmStats.firstDay) {
            fmtSeg(L(`首日（${cgmStats.firstDayDate}）`, `First day (${cgmStats.firstDayDate})`), cgmStats.firstDay);
        }
        if (cgmStats.stable) {
        ...
        sections.push(``);
        sections.push(L(`| 日期 | 条数 | 均值 | 最低 | 最高 | CV% | <3.9% | >7.8% | 备注 |`, `| Date | Count | Mean | Min | Max | CV% | <3.9% | >7.8% | Note |`));
        sections.push(`|---|---:|---:|---:|---:|---:|---:|---:|---|`);
        ...
        for (const date of Object.keys(cgmStats.daily).filter(date => recentDates.has(date)).sort()) {
            ...
            sections.push(`| ${date} | ${d.count} | ${d.mean.toFixed(2)} | ${d.min.toFixed(1)} | ${d.max.toFixed(1)} | ${d.cv.toFixed(1)} | ${d.pctBelow39.toFixed(1)} | ${d.pctAbove78.toFixed(1)} | ${tag} |`);
        }
        sections.push(``);
        sections.push(L(`**最大血糖上升**：30分钟 ${cgmStats.maxRises['30min'].rise.toFixed(1)} mmol/L, 60分钟 ${cgmStats.maxRises['60min'].rise.toFixed(1)} mmol/L, 120分钟 ${cgmStats.maxRises['120min'].rise.toFixed(1)} mmol/L`, `**Max glucose rise**: 30 min ${cgmStats.maxRises['30min'].rise.toFixed(1)} mmol/L, 60 min ${cgmStats.maxRises['60min'].rise.toFixed(1)} mmol/L, 120 min ${cgmStats.maxRises['120min'].rise.toFixed(1)} mmol/L`));
        sections.push(``);
    }
    // Blood pressure
    if (bpStats && bpStats.records.length > 0) {
        ...
        sections.push(``);
        sections.push(L(`**记录明细**（共 ${bpStats.records.length} 条；晨间=hour&lt;12，晚间=hour≥18）：`, `**Record detail** (${bpStats.records.length} records; morning=hour&lt;12, evening=hour≥18):`));
        sections.push(``);
        sections.push(L(`| 时间 | 收缩压 | 舒张压 | 备注 |`, `| Time | Systolic | Diastolic | Note |`));
        sections.push(`|---|---:|---:|---|`);
        ...
        sections.push(``);
        sections.push(L(`| 时段 | 收缩压 | 舒张压 | 条数 | <90/60 |`, `| Period | Systolic | Diastolic | Count | <90/60 |`));
        sections.push(`|---|---:|---:|---:|---:|`);
    ...
    if (weightStats && weightStats.dayCount > 0) {
        ...
        sections.push(``);
        sections.push(L(`原始称重 ${weightStats.rawCount} 条 → 趋势日 ${weightStats.dayCount} 天（每日一点：优先晨起）。`, `Raw weigh-ins ${weightStats.rawCount} records → ${weightStats.dayCount} trend days (one point/day: prefer morning).`));
        if (weightStats.latestTrend && weightStats.earliestTrend) {
            sections.push(L(`趋势体重：最早 ${weightStats.earliestTrend.weight.toFixed(1)} kg（${weightStats.earliestTrend.date}）→ 最新 ${weightStats.latestTrend.weight.toFixed(1)} kg（${weightStats.latestTrend.date}），变化 ${(weightStats.latestTrend.weight - weightStats.earliestTrend.weight).toFixed(1)} kg。`, `Trend weight: earliest ${weightStats.earliestTrend.weight.toFixed(1)} kg (${weightStats.earliestTrend.date}) → latest ${weightStats.latestTrend.weight.toFixed(1)} kg (${weightStats.latestTrend.date}), change ${(weightStats.latestTrend.weight - weightStats.earliestTrend.weight).toFixed(1)} kg.`));
        }
        if (weightStats.bodyFatDayCount > 0) {
            sections.push(L(`体脂趋势日 ${weightStats.bodyFatDayCount}：最早 ${weightStats.bodyFatEarliest?.toFixed(1)}% → 最新 ${weightStats.bodyFatLatest?.toFixed(1)}%` +
                (weightStats.bodyFatDelta != null ? `，变化 ${weightStats.bodyFatDelta.toFixed(1)} 个百分点。` : '。'), `Body fat trend days ${weightStats.bodyFatDayCount}: earliest ${weightStats.bodyFatEarliest?.toFixed(1)}% → latest ${weightStats.bodyFatLatest?.toFixed(1)}%` +
                (weightStats.bodyFatDelta != null
                    ? `, change ${weightStats.bodyFatDelta.toFixed(1)} percentage points.`
                    : '.')));
        }
        sections.push(``);
        sections.push(L(`| 日期 | 趋势体重(kg) | 晨起 | 晚间 | 体脂% | 当日条数 |`, `| Date | Trend weight (kg) | Morning | Evening | Body fat % | Count that day |`));
        sections.push(`|---|---:|---:|---:|---:|---:|`);
        ...
        for (const d of weightStats.daily.filter((x) => recentDates.has(x.date))) {
            ...
            sections.push(`| ${d.date} | ${d.trend.value.toFixed(1)} | ${morn} | ${eve} | ${fat} | ${d.allCount} |`);
        }
        sections.push(``);
    }
        ...
        sections.push(``);
        sections.push(L(`| 时间 | 体重 (kg) | 体脂% |`, `| Time | Weight (kg) | Body fat % |`));
        sections.push(`|---|---:|---:|`);
    ...
    if (Object.keys(hrvByDate).length > 0) {
        ...
        sections.push(``);
        sections.push(L(`| 日期 | 全天均值 | 夜间均值 | 最低 | 最高 | 样本数 |`, `| Date | All-day mean | Overnight mean | Min | Max | Samples |`));
        sections.push(`|---|---:|---:|---:|---:|---:|`);
        ...
        for (const date of Object.keys(hrvByDate).filter(date => recentDates.has(date)).sort()) {
            ...
            const night = h.overnightMean == null || !Number.isFinite(h.overnightMean)
                ? '—'
                : h.overnightMean.toFixed(1);
            sections.push(`| ${date} | ${h.allMean.toFixed(1)} | ${night} | ${h.min.toFixed(1)} | ${h.max.toFixed(1)} | ${h.count} |`);
        }
        sections.push(``);
    }
    // Heart rate
    if (Object.keys(data.restingHr).length > 0 || Object.keys(data.walkingHr).length > 0) {
        ...
        sections.push(L(`| 日期 | 静息心率 | 步行心率 |`, `| Date | Resting HR | Walking HR |`));
        sections.push(`|---|---:|---:|`);
    ...
    if (Object.keys(data.steps).length > 0 || Object.keys(data.sleep).length > 0) {
        ...
        const recentDates = recentDateSet(Array.from(allDates));
        sections.push(L(`| 日期 | 步数 | 睡眠(h) | 深睡(h) | REM(h) |`, `| Date | Steps | Sleep (h) | Deep (h) | REM (h) |`));
        sections.push(`|---|---:|---:|---:|---:|`);
    ...
    if (watchStats && watchStats.dayCount > 0) {
        sections.push(L(`## Apple Watch（活动 / 血氧 / 呼吸 / VO₂ / 腕温 / 呼吸紊乱）`, `## Apple Watch (activity / SpO₂ / respiration / VO₂ / wrist temp / breathing disturbances)`));
        sections.push(``);
        sections.push(L(`> 日汇总共 ${watchStats.dayCount} 天；血氧/呼吸为日内样本均值，VO₂ 为 Apple 估算，夜间心率为 0–6 点抽样；睡眠呼吸紊乱为 Watch 原始量（越高扰动相对越多，非诊断）。`, `> Daily summary: ${watchStats.dayCount} days; SpO₂/respiration are intra-day sample means; VO₂ is Apple estimate; night HR is 0–6h samples; sleep breathing disturbance is Watch raw quantity (higher ≈ more disturbance, not a diagnosis).`));
        sections.push(``);
        ...
        if (watchStats.exerciseMinMean7d != null) {
            sections.push(L(`| 日均锻炼 | ${watchStats.exerciseMinMean7d.toFixed(0)} min |`, `| Exercise daily avg | ${watchStats.exerciseMinMean7d.toFixed(0)} min |`));
        }
        if (watchStats.activeKcalMean7d != null) {
            sections.push(L(`| 日均活动消耗 | ${watchStats.activeKcalMean7d.toFixed(0)} kcal |`, `| Active energy daily avg | ${watchStats.activeKcalMean7d.toFixed(0)} kcal |`));
        }
        if (watchStats.spo2Mean7d != null) {
            sections.push(L(`| 血氧均值 / 最低 | ${watchStats.spo2Mean7d.toFixed(1)}%` +
                (watchStats.spo2Min7d != null ? ` / ${watchStats.spo2Min7d.toFixed(1)}%` : '') +
                `（${watchStats.spo2DayCount} 天） |`, `| SpO₂ mean / min | ${watchStats.spo2Mean7d.toFixed(1)}%` +
                (watchStats.spo2Min7d != null ? ` / ${watchStats.spo2Min7d.toFixed(1)}%` : '') +
                ` (${watchStats.spo2DayCount} days) |`));
        }
        if (watchStats.spo2NightMean7d != null || watchStats.spo2DayMean7d != null) {
            sections.push(L(`| 血氧 夜段(0–8) / 日段 | ` +
        ...
        if (watchStats.rrMean7d != null) {
            sections.push(L(`| 呼吸频率日均 | ${watchStats.rrMean7d.toFixed(1)} 次/分 |`, `| Respiratory rate daily avg | ${watchStats.rrMean7d.toFixed(1)} breaths/min |`));
        }
        if (watchStats.nightHrMean7d != null) {
            sections.push(L(`| 夜间心率 (0–6h) | ${watchStats.nightHrMean7d.toFixed(0)} bpm |`, `| Night HR (0–6h) | ${watchStats.nightHrMean7d.toFixed(0)} bpm |`));
        }
        if (watchStats.vo2Latest != null) {
            const d = watchStats.vo2Delta;
            sections.push(L(`| VO₂ max 最新` +
                ...
                (watchStats.vo2Earliest != null
                    ...
                    : '') +
                ` mL/kg/min（${watchStats.vo2DayCount} 天） |`, `| VO₂ max latest` +
        ...
        if (watchStats.wristTempMean7d != null) {
            sections.push(L(`| 睡眠腕温日均 | ${watchStats.wristTempMean7d.toFixed(2)} °C |`, `| Sleep wrist temp daily avg | ${watchStats.wristTempMean7d.toFixed(2)} °C |`));
        }
        if (watchStats.breathingDisturbanceMean7d != null) {
            sections.push(L(`| 睡眠呼吸紊乱日均` +
                ...
                (watchStats.breathingDisturbanceLatest != null
                    ...
                    : '') +
                `（${watchStats.breathingDisturbanceDayCount} 天） |`, `| Sleep breathing disturbance daily avg` +
        ...
        if (watchStats.daylightMinMean7d != null) {
            sections.push(L(`| 日照日均 | ${watchStats.daylightMinMean7d.toFixed(0)} min |`, `| Daylight daily avg | ${watchStats.daylightMinMean7d.toFixed(0)} min |`));
        }
        if (watchStats.standHoursMean7d != null) {
            sections.push(L(`| 站立小时日均 | ${watchStats.standHoursMean7d.toFixed(1)} h |`, `| Stand hours daily avg | ${watchStats.standHoursMean7d.toFixed(1)} h |`));
        }
        sections.push(``);
        sections.push(L(`**分日明细**（最近 ${detailDays} 天）：`, `**Daily detail** (last ${detailDays} days):`));
        sections.push(``);
        const showBdCol = (watchStats.breathingDisturbanceDayCount ?? 0) > 0;
        sections.push(L(`| 日期 | 活动kcal | 锻炼min | SpO₂均 | 夜均 | 日均 | 呼吸 | 夜间HR | VO₂ | 腕温` +
            (showBdCol ? ' | 呼吸紊乱' : '') +
        ...
        for (const d of watchStats.days.filter((x) => recentWatch.has(x.date))) {
            ...
            sections.push(`| ${d.date} | ${d.activeKcal ? d.activeKcal.toFixed(0) : '—'} | ${d.exerciseMin ? d.exerciseMin.toFixed(0) : '—'} | ${f(d.spo2Mean)} | ${f(d.spo2NightMean)} | ${f(d.spo2DayMean)} | ${f(d.rrMean)} | ${f(d.nightHrMean, 0)} | ${f(d.vo2Max)} | ${f(d.wristTempMean, 2)}` +
    ...
    if (workoutStats && workoutStats.count > 0) {
        ...
        sections.push(``);
        sections.push(L(`共 ${workoutStats.count} 场；近 30 日 ${workoutStats.count30d} 场 / ${workoutStats.durationSum30d.toFixed(0)} min` +
            ...
            (workoutStats.hrAvgMean30d != null
                ...
                : '') +
            `；近 7 日 ${workoutStats.count7d} 场 / ${workoutStats.durationSum7d.toFixed(0)} min。`, `Total ${workoutStats.count} sessions; last 30 days ${workoutStats.count30d} sessions / ${workoutStats.durationSum30d.toFixed(0)} min` +
            (workoutStats.activeKcalSum30d
                ? ` / ${workoutStats.activeKcalSum30d.toFixed(0)} kcal`
                : '') +
            (workoutStats.hrAvgMean30d != null
        ...
        if (workoutStats.byType.length) {
            ...
            sections.push(``);
            sections.push(L(`| 类型 | 场次 | 总分钟 | 活动kcal |`, `| Type | Sessions | Total min | Active kcal |`));
            sections.push(`|---|---:|---:|---:|`);
            for (const t of workoutStats.byType) {
                const label = t.activityLabel || t.activityType;
                sections.push(`| ${label} | ${t.count} | ${t.durationMin.toFixed(0)} | ${t.activeKcal.toFixed(0)} |`);
            }
        }
        sections.push(``);
        sections.push(L(`**最近会话**（最多 40 场）：`, `**Recent sessions** (up to 40):`));
        sections.push(``);
        sections.push(L(`| 开始 | 类型 | 分钟 | kcal | 距离km | HR均 | HR最大 | METs |`, `| Start | Type | Min | kcal | Dist km | HR avg | HR max | METs |`));
        sections.push(`|---|---|---:|---:|---:|---:|---:|---:|`);
        const recentW = workoutStats.sessions.slice(-40);
        for (const s of recentW) {
            const label = s.activityLabel || s.activityType;
            sections.push(`| ${s.startDate.slice(0, 16)} | ${label} | ${s.durationMin.toFixed(1)} | ${s.activeKcal != null ? s.activeKcal.toFixed(0) : '—'} | ${s.distanceKm != null ? s.distanceKm.toFixed(2) : '—'} | ${s.hrAvg != null ? s.hrAvg.toFixed(0) : '—'} | ${s.hrMax != null ? s.hrMax.toFixed(0) : '—'} | ${s.avgMets != null ? s.avgMets.toFixed(1) : '—'} |`);
        }
        sections.push(``);
    }
    // Weekly recovery
    if (recoveryWeek) {
        ...
        sections.push(``);
        sections.push(L(`> 启发式评分，非诊断；截止 ${rw.weekEnd}。状态：${rw.statusLabel}`, `> Heuristic score, not a diagnosis; through ${rw.weekEnd}. Status: ${rw.statusLabel}`));
        sections.push(``);
        sections.push(L(`| 指标 | 值 |`, `| Metric | Value |`));
        sections.push(`|---|---|`);
        if (rw.recoveryScore != null) {
            sections.push(L(`| 恢复分 | ${rw.recoveryScore} / 100 |`, `| Recovery score | ${rw.recoveryScore} / 100 |`));
        }
        if (rw.loadScore != null) {
            sections.push(L(`| 负荷分 | ${rw.loadScore} / 100 |`, `| Load score | ${rw.loadScore} / 100 |`));
        }
        if (rw.baselineRecoveryMedian != null) {
            sections.push(L(`| 近几周恢复分中位（个人基线） | ${rw.baselineRecoveryMedian} |`, `| Recent weeks recovery median (personal baseline) | ${rw.baselineRecoveryMedian} |`));
        }
        if (rw.vsBaselineDelta != null) {
            const sign = rw.vsBaselineDelta > 0 ? '+' : '';
            sections.push(L(`| 相对基线 | ${sign}${rw.vsBaselineDelta}` +
        ...
        if (rw.hrvMean7d != null) {
            sections.push(L(`| HRV 日均 | ${rw.hrvMean7d.toFixed(1)} ms |`, `| HRV daily avg | ${rw.hrvMean7d.toFixed(1)} ms |`));
        }
        if (rw.nightHrMean7d != null) {
            sections.push(L(`| 夜间心率 | ${rw.nightHrMean7d.toFixed(0)} bpm |`, `| Night HR | ${rw.nightHrMean7d.toFixed(0)} bpm |`));
        }
        if (rw.restingHrMean7d != null) {
            sections.push(L(`| 静息心率 | ${rw.restingHrMean7d.toFixed(0)} bpm |`, `| Resting HR | ${rw.restingHrMean7d.toFixed(0)} bpm |`));
        }
        if (rw.exerciseMinMean7d != null) {
            sections.push(L(`| 锻炼日均 | ${rw.exerciseMinMean7d.toFixed(0)} min |`, `| Exercise daily avg | ${rw.exerciseMinMean7d.toFixed(0)} min |`));
        }
        sections.push(L(`| Workout | ${rw.workoutCount7d} 场 / ${rw.workoutDuration7d.toFixed(0)} min |`, `| Workout | ${rw.workoutCount7d} sessions / ${rw.workoutDuration7d.toFixed(0)} min |`));
        if (rw.sleepMean7d != null) {
            sections.push(L(`| 睡眠日均 | ${rw.sleepMean7d.toFixed(2)} h |`, `| Sleep daily avg | ${rw.sleepMean7d.toFixed(2)} h |`));
        }
        if (rw.stepsMean7d != null) {
            sections.push(L(`| 步数日均 | ${Math.round(rw.stepsMean7d)} |`, `| Steps daily avg | ${Math.round(rw.stepsMean7d)} |`));
        }
        if (rw.standHoursMean7d != null) {
            sections.push(L(`| 站立小时日均 | ${rw.standHoursMean7d.toFixed(1)} |`, `| Stand hours daily avg | ${rw.standHoursMean7d.toFixed(1)} |`));
        }
        if (rw.daylightMinMean7d != null) {
            sections.push(L(`| 日照日均 | ${rw.daylightMinMean7d.toFixed(0)} min |`, `| Daylight daily avg | ${rw.daylightMinMean7d.toFixed(0)} min |`));
        }
        if (rw.spo2NightMean7d != null) {
            sections.push(L(`| 夜段血氧 | ${rw.spo2NightMean7d.toFixed(1)}% |`, `| Night SpO₂ | ${rw.spo2NightMean7d.toFixed(1)}% |`));
        }
        sections.push(``);
    }
    ...
    if (recoveryWeeks && recoveryWeeks.length > 0) {
        ...
        sections.push(``);
        sections.push(L(`> 启发式评分，非诊断；共 ${recoveryWeeks.length} 周样本，下表最近 ${recent.length} 周（最旧→最新）。`, `> Heuristic score, not a diagnosis; ${recoveryWeeks.length} week samples total; table shows last ${recent.length} weeks (oldest → newest).`));
        sections.push(``);
        sections.push(L(`| 周末 | 恢复分 | 负荷分 | HRV | 夜心 | 锻炼 | 睡眠 | Workout |`, `| Week end | Recovery | Load | HRV | Night HR | Exercise | Sleep | Workout |`));
        sections.push(`|---|---:|---:|---:|---:|---:|---:|---:|`);
        for (const p of recent) {
            sections.push(`| ${p.weekEnd} | ${p.recoveryScore != null ? p.recoveryScore : '—'} | ${p.loadScore != null ? p.loadScore : '—'} | ${p.hrvMean7d != null ? p.hrvMean7d.toFixed(0) : '—'} | ${p.nightHrMean7d != null ? p.nightHrMean7d.toFixed(0) : '—'} | ${p.exerciseMinMean7d != null ? p.exerciseMinMean7d.toFixed(0) : '—'} | ${p.sleepMean7d != null ? p.sleepMean7d.toFixed(1) : '—'} | ${p.workoutCount7d} |`);
        }
        sections.push(``);
    }
    // ECG
    if (ecgStats && ecgStats.count > 0) {
        ...
        sections.push(``);
        sections.push(L(`共 ${ecgStats.count} 份（窦性 ${ecgStats.sinusCount} · 高心率 ${ecgStats.highHrCount} · 结果不佳 ${ecgStats.inconclusiveCount} · 其他 ${ecgStats.otherCount}）`, `Total ${ecgStats.count} reports (sinus ${ecgStats.sinusCount} · high HR ${ecgStats.highHrCount} · inconclusive ${ecgStats.inconclusiveCount} · other ${ecgStats.otherCount})`));
        if (ecgStats.highHrCount > 0) {
            ...
            const hourBits = (ecgStats.highHrByHour || [])
                ...
                .filter(Boolean);
            sections.push(``);
            sections.push(L(`高心率关联：训练±2h ${near}/${hh}（${nearPct}%）· 非运动窗 ${rest}/${hh}` +
                (hourBits.length ? `；小时分布 ${hourBits.join('、')}` : ''), `High-HR association: workout ±2h ${near}/${hh} (${nearPct}%) · non-exercise window ${rest}/${hh}` +
            ...
            if (lowAct > 0 || highAct > 0) {
                sections.push(L(`高心率×活动日：低活动日 ${lowAct} 份 · 高活动/训练邻域 ${highAct} 份（低活动≈步数<3000 且锻炼少）`, `High HR × activity day: low-activity days ${lowAct} reports · high activity / near workout ${highAct} reports (low activity ≈ steps <3000 and little exercise)`));
            }
            if (ecgStats.recentHighHr && ecgStats.recentHighHr.length) {
                sections.push(L(`最近高心率时刻：${ecgStats.recentHighHr.map((d) => String(d).slice(0, 16)).join(' · ')}`, `Recent high-HR times: ${ecgStats.recentHighHr.map((d) => String(d).slice(0, 16)).join(' · ')}`));
            }
        }
        sections.push(``);
        ...
        if (ecgStats.latest) {
            sections.push(``);
            sections.push(L(`最近：${ecgStats.latest.datetime} — **${ecgStats.latest.classification}**` +
                (ecgStats.latest.device ? `（${ecgStats.latest.device}）` : ''), `Latest: ${ecgStats.latest.datetime} — **${ecgStats.latest.classification}**` +
                (ecgStats.latest.device ? ` (${ecgStats.latest.device})` : '')));
        }
        sections.push(``);
        sections.push(L(`**明细**（最近 30 份）：`, `**Detail** (last 30 reports):`));
        sections.push(``);
        sections.push(L(`| 时间 | 分类 | 设备 |`, `| Time | Classification | Device |`));
        sections.push(`|---|---|---|`);
        for (const e of data.ecg.slice(-30)) {
            sections.push(`| ${e.datetime} | ${e.classification} | ${e.device || '—'} |`);
        }
        sections.push(``);
    }
        ...
        sections.push(L(`## ECG 心电图`, `## ECG`));
        sections.push(``);
        sections.push(L(`共 ${data.ecg.length} 份 ECG`, `Total ${data.ecg.length} ECG reports`));
        sections.push(``);
    }
    let md = sections.join('\n');
    // zh-TW: traditionalize ##/### section headers only (long medical body stays zh-CN)
    if ((0, locale_1.normalizeLocale)(options?.locale) === 'zh-TW') {
        md = (0, zh_tw_map_1.traditionalizeMarkdownHeadings)(md);
    }
    return md;
}
function combineContextAndData(analysis, userContext, options) {
    const localeOpts = { locale: (0, locale_1.normalizeLocale)(options?.locale) };
    const insightsSection = (0, insights_1.formatInsightsForLLM)((0, insights_1.buildInsightBullets)(analysis, localeOpts), localeOpts);
    const dataSection = formatAnalysisForLLM(analysis, localeOpts);
    const ctxSection = formatUserContext(userContext, localeOpts);
    const signalsSection = (0, signals_1.formatCrossSignalsForLLM)((0, signals_1.detectCrossSignals)(analysis, localeOpts), localeOpts);
    const parts = [ctxSection, insightsSection, dataSection, signalsSection].filter((s) => s && s.trim());
    return parts.join('\n');
}
/**
...
function generateLLMPrompt(analysis, userContext, options) {
    const locale = (0, locale_1.normalizeLocale)(options?.locale);
    const dataSection = combineContextAndData(analysis, userContext, { locale });
    const template = locale === 'en' ? exports.MAIN_PROMPT_TEMPLATE_EN : exports.MAIN_PROMPT_TEMPLATE;
    return template
