...
const {
  ...
  EXT_SOURCE_NAME,
  EXT_SOURCE_BATCH_IDS,
  opaqueImportBatchId,
  isOpaqueAnonymousBatchId,
...
<Record type="HKCategoryTypeIdentifierSleepAnalysis" value="HKCategoryValueSleepAnalysisAsleepCore" startDate="2026-07-01 23:00:00 +0800" sourceName="Apple Watch"/>
  ...
  test('createImportBatchId / normalizeImportBatch round-trip', () => {
    ...
    assert.equal(rec.files[0].digestScope, 'full');
    assert.equal(rec.files[0].bytesHashed, 1234);
    ...
    // legacy sha without scope → prefix_1mib (must not claim full-file integrity)
    const legacy = normalizeImportBatch({
      id: 'batch_1_legacy',
      source: 'hae',
      createdAt: '2026-07-30T02:00:00.000Z',
      files: [{ name: 'big.json', bytes: 9e6, sha256: 'aa'.repeat(32) }],
      stats: { totalAdded: 0, totalUpdated: 0, totalSkipped: 0 },
    });
    assert.ok(legacy);
    assert.equal(legacy.files[0].digestScope, 'prefix_1mib');
  ...
  test('formatProvenanceAppendixMarkdown + clinical opt-in', () => {
    const batch = normalizeImportBatch({
      ...
      files: [
        ...
        },
        {
          ...
          bytes: 5_000_000,
          sha256: 'cc'.repeat(32),
          digestScope: 'prefix_1mib',
          bytesHashed: 1048576,
        },
      ],
      totalBytes: 5_000_500,
    ...
    assert.match(appendix, /本报告关联|非全部导入历史/);
    ...
    assert.match(appendix, /SHA-256=/); // full file
    assert.match(appendix, /前 1MiB|first 1MiB/i); // partial must be labeled

    const empty = formatProvenanceAppendixMarkdown([], { locale: 'en' });
    assert.match(empty, /linked to this analysis|not full import history/i);
  ...
  // -------------------------------------------------------------------------
  suite('FHIR R4-shaped local export (v1.48)');

  test('buildFhirExportBundle on SAMPLE_XML → Bundle + Observations', () => {
    const analysis = analyzeAll(parseHealthXml(SAMPLE_XML));
    ...
    assert.ok(Array.isArray(result.bundle.entry));
    const obs = result.bundle.entry
      .map((e) => e.resource)
      .filter((r) => r && r.resourceType === 'Observation');
    assert.ok(obs.length >= 1, 'expected at least one Observation');
    assert.ok(result.counts.observations >= 1);
    assert.ok(typeof result.json === 'string' && result.json.includes('"Bundle"'));
    assert.ok(Array.isArray(result.notes) && result.notes.some((n) => /experimental FHIR/i.test(n)));
    assert.equal(typeof FHIR_EXPORT_PROFILE, 'string');
    assert.match(FHIR_R4, /hl7\.org\/fhir/);
    // meta tag / profile
    const tags = (result.bundle.meta && result.bundle.meta.tag) || [];
    assert.ok(tags.some((t) => t.code === FHIR_EXPORT_PROFILE || t.code === PROVENANCE_RULE_VERSION));
    // v1.52.1: fullUrl must be urn:uuid:
    for (const e of result.bundle.entry) {
      assert.match(String(e.fullUrl || ''), /^urn:uuid:[0-9a-f-]{36}$/i);
    }
    assert.ok(result.validation && result.validation.ok, result.validation && result.validation.issues.join('; '));
  });

  test('BP observation has components or systolic', () => {
    const analysis = analyzeAll(parseHealthXml(SAMPLE_XML));
    const result = buildFhirExportBundle(analysis);
    const bpObs = result.bundle.entry
      .map((e) => e.resource)
      .find(
        (r) =>
          r &&
          r.resourceType === 'Observation' &&
          String(r.id || '').startsWith('obs-bp-')
      );
    assert.ok(bpObs, 'expected BP observation');
    const hasComponents =
      Array.isArray(bpObs.component) &&
      bpObs.component.some(
        (c) =>
          c &&
          c.code &&
          Array.isArray(c.code.coding) &&
          c.code.coding.some((x) => x.code === '8480-6' || x.code === '8462-4')
      );
    const hasSysValue =
      bpObs.valueQuantity &&
      bpObs.code &&
      Array.isArray(bpObs.code.coding) &&
      bpObs.code.coding.some((x) => x.code === '8480-6');
    assert.ok(hasComponents || hasSysValue, 'BP should have sys/dia components or systolic');
    if (hasComponents) {
      const sys = bpObs.component.find(
        (c) => c.code.coding && c.code.coding.some((x) => x.code === '8480-6')
      );
      assert.equal(sys.valueQuantity.value, 120);
      const dia = bpObs.component.find(
        (c) => c.code.coding && c.code.coding.some((x) => x.code === '8462-4')
      );
      assert.equal(dia.valueQuantity.value, 80);
    }
  });

  test('with importBatches → Provenance with target and entity batch id', () => {
    const analysis = analyzeAll(parseHealthXml(SAMPLE_XML));
    const batch = normalizeImportBatch({
      id: 'batch_1720000000000_fhir01',
      source: 'hae',
      createdAt: '2026-07-30T05:00:00.000Z',
      files: [{ name: 'BloodGlucose.json', bytes: 100 }],
      stats: { totalAdded: 5, totalUpdated: 0, totalSkipped: 0 },
    });
    assert.ok(batch);
    const result = buildFhirExportBundle(analysis, {
    ...
    const fullUrls = new Set(
      result.bundle.entry.map((e) => String(e.fullUrl || '')).filter(Boolean)
    );
    const prov = result.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && r.resourceType === 'Provenance');
    assert.ok(prov, 'expected Provenance resource');
    assert.ok(Array.isArray(prov.target) && prov.target.length >= 1);
    // targets must be Bundle entry fullUrls (urn:uuid:), not relative Resource/id
    for (const t of prov.target) {
      assert.match(String(t.reference || ''), /^urn:uuid:/i);
      assert.ok(fullUrls.has(String(t.reference)));
    }
    assert.ok(Array.isArray(prov.entity) && prov.entity.length >= 1);
    const entityIds = prov.entity.map(
      (en) => en.what && en.what.identifier && en.what.identifier.value
    );
    assert.ok(entityIds.includes('batch_1720000000000_fhir01'));
    assert.equal(result.counts.provenances, 1);
    // no domainSourceBatches → coarse fallback note
    assert.ok(
      result.notes.some((n) => /domain map unavailable|coarse provenance/i.test(n)),
      'expected coarse provenance note when domain map missing'
    );
    assert.ok(result.validation && result.validation.ok, (result.validation.issues || []).join('; '));
  });

  test('v1.53: fine-grained Provenance per domain batch + Observation source-batch-ids extension', () => {
    const data = createEmptyData();
    data.cgm.push({ datetime: '2026-07-10 08:15:00 +0800', value: 5.5 });
    data.cgm.push({ datetime: '2026-07-10 12:00:00 +0800', value: 6.1 });
    data.bloodPressure.push({
      datetime: '2026-07-10 07:00:00 +0800',
      date: '2026-07-10',
      systolic: 120,
      diastolic: 80,
    });
    ...
    data.dataAvailability.hasBloodPressure = true;
    const analysis = analyzeAll(data);
    analysis.domainSourceBatches = {
      cgm: ['batch_1720000000000_cgm01'],
      bloodPressure: ['batch_1720000000000_bp01'],
    };
    const batchCgm = normalizeImportBatch({
      id: 'batch_1720000000000_cgm01',
      source: 'hae',
      createdAt: '2026-07-30T05:00:00.000Z',
      files: [{ name: 'BloodGlucose.json', bytes: 100 }],
      stats: {
        ...
        totalUpdated: 0,
        totalSkipped: 0,
        byDomain: { cgm: { added: 2, updated: 0, skipped: 0 } },
      },
    });
    const batchBp = normalizeImportBatch({
      id: 'batch_1720000000000_bp01',
      source: 'hae',
      createdAt: '2026-07-30T06:00:00.000Z',
      files: [{ name: 'BloodPressure.json', bytes: 50 }],
      stats: {
        totalAdded: 1,
        totalUpdated: 0,
        totalSkipped: 0,
        byDomain: { bloodPressure: { added: 1, updated: 0, skipped: 0 } },
      },
    });
    assert.ok(batchCgm && batchBp);
    const result = buildFhirExportBundle(analysis, {
      includeProvenance: true,
      importBatches: [batchCgm, batchBp],
    });
    assert.equal(result.counts.provenances, 2, 'expected one Provenance per batch');
    assert.ok(
      result.notes.some((n) => /fine-grained provenance/i.test(n)),
      'expected fine-grained provenance note'
    );

    const entryByFullUrl = new Map(
      result.bundle.entry.map((e) => [String(e.fullUrl || ''), e.resource])
    );
    const provenances = result.bundle.entry
      .map((e) => e.resource)
      .filter((r) => r && r.resourceType === 'Provenance');
    ...
    for (const p of provenances) {
      const eid =
        ...
        p.entity[0] &&
        p.entity[0].what &&
        p.entity[0].what.identifier &&
        p.entity[0].what.identifier.value;
      assert.ok(eid, 'each fine-grained Provenance should have one batch entity');
      assert.equal(p.entity.length, 1);
      assert.match(String(p.id || ''), /^prov-batch-/);
      provByEntity[eid] = p;
    }
    assert.ok(provByEntity['batch_1720000000000_cgm01']);
    assert.ok(provByEntity['batch_1720000000000_bp01']);

    const targetIds = (prov) =>
      (prov.target || []).map((t) => {
        const res = entryByFullUrl.get(String(t.reference));
        return res && res.id != null ? String(res.id) : '';
      });

    const cgmTargets = targetIds(provByEntity['batch_1720000000000_cgm01']);
    const bpTargets = targetIds(provByEntity['batch_1720000000000_bp01']);
    assert.ok(cgmTargets.length >= 1);
    assert.ok(bpTargets.length >= 1);
    assert.ok(
      cgmTargets.every((id) => id.startsWith('obs-glucose-')),
      `cgm provenance should only target glucose: ${cgmTargets.join(',')}`
    );
    assert.ok(
      bpTargets.every((id) => id.startsWith('obs-bp-')),
      `bp provenance should only target bp: ${bpTargets.join(',')}`
    );
    // no cross-contamination
    assert.ok(!cgmTargets.some((id) => id.startsWith('obs-bp-')));
    assert.ok(!bpTargets.some((id) => id.startsWith('obs-glucose-')));

    // Observation extension source-batch-ids
    const glucoseObs = result.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && String(r.id || '').startsWith('obs-glucose-'));
    assert.ok(glucoseObs);
    const gExt = (glucoseObs.extension || []).find(
      (x) => x && x.url === 'urn:health-analyzer:extension:source-batch-ids'
    );
    assert.ok(gExt, 'glucose Observation should carry source-batch-ids extension');
    assert.equal(gExt.valueString, 'batch_1720000000000_cgm01');

    const bpObs = result.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && String(r.id || '').startsWith('obs-bp-'));
    assert.ok(bpObs);
    const bExt = (bpObs.extension || []).find(
      (x) => x && x.url === 'urn:health-analyzer:extension:source-batch-ids'
    );
    assert.ok(bExt);
    assert.equal(bExt.valueString, 'batch_1720000000000_bp01');

    assert.ok(result.validation && result.validation.ok, (result.validation.issues || []).join('; '));
  });

  test('v1.52.1: daily aggregates use effectivePeriod; CGM/BP use effectiveDateTime', () => {
    const data = createEmptyData();
    data.steps['2026-07-10'] = { watch: 8000, iphone: 0, max: 8000 };
    data.restingHr['2026-07-10'] = 58;
    data.sleep['2026-07-10'] = { total: 7, deep: 1, rem: 1.5, core: 4, awake: 0.5 };
    data.cgm.push({ datetime: '2026-07-10 08:15:00 +0800', value: 5.5 });
    data.bloodPressure.push({
      datetime: '2026-07-10 07:00:00 +0800',
      date: '2026-07-10',
    ...
    assert.ok(byId['obs-steps-0'].effectivePeriod);
    assert.ok(byId['obs-steps-0'].effectivePeriod.start);
    assert.ok(byId['obs-steps-0'].effectivePeriod.end);
    assert.equal(byId['obs-steps-0'].effectiveDateTime, undefined);
    assert.ok(byId['obs-resting-hr-0'].effectivePeriod);
    assert.ok(byId['obs-sleep-0'].effectivePeriod);
    assert.ok(byId['obs-glucose-0'].effectiveDateTime);
    assert.ok(byId['obs-bp-0'].effectiveDateTime);
  });

  test('CGM capped: 100 points maxCgm 10 → glucose observations ≤ 10', () => {
    const data = createEmptyData();
    for (let i = 0; i < 100; i++) {
      const h = String(Math.floor(i / 4)).padStart(2, '0');
      const m = String((i % 4) * 15).padStart(2, '0');
      // spread across a few days within window
      const day = 1 + (i % 5);
      const d = `2026-07-0${day}`;
      data.cgm.push({
        datetime: `${d} ${h}:${m}:00 +0800`,
        value: 5 + (i % 10) * 0.1,
      });
    }
    data.dataAvailability.hasCgm = true;
    const analysis = analyzeAll(data);
    const result = buildFhirExportBundle(analysis, { maxCgm: 10 });
    assert.ok(result.counts.byType.glucose <= 10);
    assert.equal(result.counts.byType.glucose, 10);
    assert.ok(result.notes.some((n) => /CGM capped/i.test(n)));
    // toIsoDateTime smoke
    const iso = toIsoDateTime('2026-07-01 08:00:00 +0800');
    assert.match(iso, /2026-07-01T/);
  });

  test('FHIR v1.49: SpO2 + sleep Observations + optional DocumentReference', () => {
    const data = createEmptyData();
    data.sleep['2026-07-10'] = { total: 7.2, deep: 1.1, rem: 1.5, core: 4.0, awake: 0.4 };
    data.sleep['2026-07-11'] = { total: 6.8, deep: 1.0, rem: 1.4, core: 3.9, awake: 0.3 };
    data.dataAvailability.hasSleep = true;
    // minimal watch daily for SpO2 via analyzeAll
    const emptyWatch = () => ({
      activeKcal: 0,
      exerciseMin: 0,
      standMin: 0,
    ...
    // inject via watchDaily with spo2 samples so calcWatchStats computes means
    data.watchDaily['2026-07-10'] = {
      ...emptyWatch(),
      spo2Sum: 96 * 10,
      spo2Count: 10,
      spo2Min: 94,
    };
    data.watchDaily['2026-07-11'] = {
      ...emptyWatch(),
      spo2Sum: 97 * 8,
      spo2Count: 8,
      spo2Min: 95,
    ...
    const analysis = analyzeAll(data);
    assert.ok(analysis.watchStats && analysis.watchStats.days.length >= 1);

    const result = buildFhirExportBundle(analysis);
    assert.ok(result.counts.byType.sleep >= 1, 'expected sleep observations');
    assert.ok(result.counts.byType.spo2 >= 1, 'expected spo2 observations');
    const sleepObs = result.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && String(r.id || '').startsWith('obs-sleep-'));
    assert.ok(sleepObs && sleepObs.valueQuantity && sleepObs.valueQuantity.value === 7.2);
    assert.ok(Array.isArray(sleepObs.component) && sleepObs.component.length >= 1);

    const spo2Obs = result.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && String(r.id || '').startsWith('obs-spo2-'));
    assert.ok(spo2Obs && spo2Obs.valueQuantity);

    // DocumentReference opt-in
    const withDoc = buildFhirExportBundle(analysis, {
      includeClinicalDocument: true,
      clinicalMarkdown: '# 就诊复盘\n\n> 非诊断\n',
      clinicalHtml: '<h1>clinic</h1>',
    });
    assert.equal(withDoc.counts.documentReferences, 1);
    const doc = withDoc.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && r.resourceType === 'DocumentReference');
    assert.ok(doc);
    assert.ok(Array.isArray(doc.content) && doc.content.length >= 1);
    assert.ok(doc.content[0].attachment && doc.content[0].attachment.data);

    // Provenance targets include DocumentReference when present
    const withBoth = buildFhirExportBundle(analysis, {
      includeClinicalDocument: true,
      clinicalMarkdown: 'note',
      includeProvenance: true,
      importBatches: [
        normalizeImportBatch({
          id: 'batch_1720000000000_doc01',
          source: 'hae',
          createdAt: '2026-07-30T06:00:00.000Z',
          files: [{ name: 'x.json', bytes: 1 }],
          stats: { totalAdded: 1, totalUpdated: 0, totalSkipped: 0 },
        }),
      ].filter(Boolean),
    });
    const fullUrls = new Set(
      withBoth.bundle.entry.map((e) => String(e.fullUrl || '')).filter(Boolean)
    );
    const prov = withBoth.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && r.resourceType === 'Provenance');
    assert.ok(prov);
    // DocumentReference targets use urn:uuid: fullUrl (v1.52.1)
    const docEntry = withBoth.bundle.entry.find(
      (e) => e.resource && e.resource.resourceType === 'DocumentReference'
    );
    assert.ok(docEntry && fullUrls.has(String(docEntry.fullUrl)));
    assert.ok(
      prov.target.some((t) => String(t.reference) === String(docEntry.fullUrl))
    );
  });

  test('FHIR v1.52: night HR + respiratory rate Observations', () => {
    const data = createEmptyData();
    const emptyWatch = () => ({
      activeKcal: 40,
      exerciseMin: 5,
    ...
    data.watchDaily['2026-07-14'] = { ...emptyWatch() };
    data.watchDaily['2026-07-15'] = {
      ...emptyWatch(),
      rrSum: 15 * 4,
      rrCount: 4,
      nightHrSum: 50 * 5,
    ...
    const analysis = analyzeAll(data);
    const result = buildFhirExportBundle(analysis);
    assert.ok(result.counts.byType.nightHeartRate >= 1);
    assert.ok(result.counts.byType.respiratoryRate >= 1);
    const nhr = result.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && String(r.id || '').startsWith('obs-night-hr-'));
    assert.ok(nhr && nhr.valueQuantity && /night/i.test(JSON.stringify(nhr.note || [])));
    const rr = result.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && String(r.id || '').startsWith('obs-rr-'));
    assert.ok(rr && rr.valueQuantity && rr.code.coding.some((c) => c.code === '9279-1'));
    assert.ok(result.validation && result.validation.ok);
  });

  test('FHIR v1.51: wrist temp + validateFhirExportBundle self-check', () => {
    const data = createEmptyData();
    const emptyWatch = () => ({
      activeKcal: 50,
      exerciseMin: 10,
    ...
    data.watchDaily['2026-07-12'] = { ...emptyWatch() };
    data.watchDaily['2026-07-13'] = {
      ...emptyWatch(),
      wristTempSum: 36.6 * 3,
      wristTempCount: 3,
    };
    ...
    const analysis = analyzeAll(data);
    const result = buildFhirExportBundle(analysis);
    assert.ok(result.counts.byType.wristTemperature >= 1);
    const wtObs = result.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && String(r.id || '').startsWith('obs-wrist-temp-'));
    // absolute-range values may use Cel; coding is experimental local, not LOINC 8310-5
    assert.ok(wtObs && wtObs.valueQuantity);
    assert.ok(
      wtObs.code &&
        Array.isArray(wtObs.code.coding) &&
        wtObs.code.coding.some((c) => c.code === 'apple_sleeping_wrist_temperature')
    );
    assert.ok(wtObs.effectivePeriod);
    assert.ok(result.validation);
    assert.equal(result.validation.ok, true);
    assert.equal(result.validation.issues.length, 0);
    assert.ok(result.notes.some((n) => /structure self-check: ok/i.test(n)));

    // broken bundle fails validation
    const bad = validateFhirExportBundle({ resourceType: 'Bundle', type: 'collection', entry: [] });
    assert.equal(bad.ok, false);
    assert.ok(bad.issues.some((i) => /empty/i.test(i)));
  });

  test('FHIR v1.50: VO2 + breathing disturbance + AGP SVG DocumentReference', () => {
    const data = createEmptyData();
    const emptyWatch = () => ({
      activeKcal: 100,
      exerciseMin: 20,
    ...
    // Dense CGM for AGP sufficient window (reuse pattern from cgm14 tests)
    for (let day = 0; day < 14; day++) {
      const d = addDaysIso('2026-07-20', -(13 - day));
      for (let min = 0; min < 24 * 60; min += 15) {
        const hh = String(Math.floor(min / 60)).padStart(2, '0');
        const mm = String(min % 60).padStart(2, '0');
        data.cgm.push({
          datetime: `${d} ${hh}:${mm}:00 +0800`,
          value: 5.5 + (min % 120 === 0 ? 0.3 : 0),
        });
      }
    }
    data.dataAvailability.hasCgm = true;
    ...
    const analysis = analyzeAll(data);
    analysis.dateRange = { start: '2026-07-01', end: '2026-07-20' };

    const result = buildFhirExportBundle(analysis, { includeAgpSvg: true });
    assert.ok(result.counts.byType.vo2Max >= 1, 'expected VO2 observations');
    assert.ok(
      result.counts.byType.breathingDisturbance >= 1,
      'expected breathing disturbance observations'
    );
    assert.equal(result.counts.byType.agpSvg, 1, 'expected AGP SVG DocumentReference');
    const agpDoc = result.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && r.id === 'docref-agp-svg-1');
    assert.ok(agpDoc);
    assert.ok(
      agpDoc.content &&
        agpDoc.content[0] &&
        agpDoc.content[0].attachment &&
        agpDoc.content[0].attachment.contentType === 'image/svg+xml'
    );
    assert.ok(agpDoc.content[0].attachment.data);
  });

  test('FHIR v1.54: golden structure fixture validates offline (urn:uuid + period/datetime + Provenance)', () => {
    const fs = require('node:fs');
    const path = require('node:path');
    const fixturePath = path.join(__dirname, 'fixtures', 'fhir-bundle-structure.json');
    ...
    assert.ok(Array.isArray(bundle.entry) && bundle.entry.length >= 3);

    const URN =
      /^urn:uuid:[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
    const fullUrls = bundle.entry.map((e) => String(e.fullUrl || ''));
    for (const u of fullUrls) {
      assert.match(u, URN, `fullUrl must be urn:uuid: got ${u}`);
    }

    const steps = bundle.entry
      .map((e) => e.resource)
      .find((r) => r && String(r.id || '').startsWith('obs-steps-'));
    assert.ok(steps && steps.effectivePeriod && steps.effectivePeriod.start && steps.effectivePeriod.end);
    assert.ok(!steps.effectiveDateTime);

    const glucose = bundle.entry
      .map((e) => e.resource)
      .find((r) => r && String(r.id || '').startsWith('obs-glucose-'));
    assert.ok(glucose && glucose.effectiveDateTime);

    const prov = bundle.entry.map((e) => e.resource).find((r) => r && r.resourceType === 'Provenance');
    assert.ok(prov && Array.isArray(prov.target) && prov.target.length >= 2);
    const fullUrlSet = new Set(fullUrls);
    for (const t of prov.target) {
      assert.ok(fullUrlSet.has(String(t.reference)), `target ${t.reference} must match entry fullUrl`);
    }

    const validation = validateFhirExportBundle(bundle);
    assert.equal(validation.ok, true, (validation.issues || []).join('; '));
    assert.equal(validation.issues.length, 0);
    assert.ok((validation.resourceCounts.Observation || 0) >= 2);
    assert.ok((validation.resourceCounts.Provenance || 0) >= 1);
  });

  test('v1.55: optional Patient pseudonym + subject fullUrl; default no identity', () => {
    const analysis = analyzeAll(parseHealthXml(SAMPLE_XML));
    assert.match(FHIR_EXPORT_PROFILE, /v1\.\d/);

    // default: no Patient, no subject on Observations
    const off = buildFhirExportBundle(analysis);
    const patientsOff = off.bundle.entry
      .map((e) => e.resource)
      .filter((r) => r && r.resourceType === 'Patient');
    assert.equal(patientsOff.length, 0);
    assert.equal(off.counts.patients, 0);
    const obsOff = off.bundle.entry
      .map((e) => e.resource)
      .filter((r) => r && r.resourceType === 'Observation');
    assert.ok(obsOff.length >= 1);
    assert.ok(
      obsOff.every((o) => !o.subject),
      'default export must not stamp subject'
    );
    assert.ok(off.validation && off.validation.ok, (off.validation.issues || []).join('; '));

    // includePatient: Patient resource + subject.reference = Patient fullUrl
    const on = buildFhirExportBundle(analysis, {
      includePatient: true,
      patientDisplay: '匿名-A',
      patientGender: 'other',
      patientBirthYear: 1988,
      includeClinicalDocument: true,
      clinicalMarkdown: '# note\n',
    });
    assert.equal(on.counts.patients, 1);
    // Patient is first Bundle entry
    assert.equal(on.bundle.entry[0].resource.resourceType, 'Patient');
    const patientEntry = on.bundle.entry.find(
      (e) => e.resource && e.resource.resourceType === 'Patient'
    );
    assert.ok(patientEntry, 'expected Patient entry');
    assert.match(String(patientEntry.fullUrl || ''), /^urn:uuid:/i);
    assert.equal(patientEntry.resource.id, 'patient-local-1');
    assert.equal(patientEntry.resource.meta && patientEntry.resource.meta.source, 'urn:health-analyzer:local');
    assert.ok(
      patientEntry.resource.name &&
        patientEntry.resource.name[0] &&
        patientEntry.resource.name[0].text === '匿名-A'
    );
    // v1.56: default no fixed identifier (merge risk)
    assert.ok(
      !patientEntry.resource.identifier ||
        !patientEntry.resource.identifier.length ||
        !patientEntry.resource.identifier.some(
          (id) => id && id.value === 'local-patient'
        ),
      'must not use fixed local-patient identifier'
    );
    assert.equal(patientEntry.resource.gender, 'other');
    // FHIR date year-only precision (not fabricated Jan 1)
    assert.equal(patientEntry.resource.birthDate, '1988');
    assert.ok(
      Array.isArray(patientEntry.resource.extension) &&
        patientEntry.resource.extension.some(
          (x) => x && /year-only|pseudonym|not a verified/i.test(String(x.valueString || ''))
        )
    );

    const patientFullUrl = String(patientEntry.fullUrl);
    const fullUrlSet = new Set(on.bundle.entry.map((e) => String(e.fullUrl || '')));
    assert.ok(fullUrlSet.has(patientFullUrl));

    const withSubject = on.bundle.entry
      .map((e) => e.resource)
      .filter(
        (r) =>
          r &&
          (r.resourceType === 'Observation' || r.resourceType === 'DocumentReference')
      );
    assert.ok(withSubject.length >= 1);
    for (const r of withSubject) {
      assert.ok(r.subject, `${r.resourceType}/${r.id} missing subject`);
      assert.equal(
        String(r.subject.reference),
        patientFullUrl,
        `${r.resourceType}/${r.id} subject must be Patient fullUrl`
      );
      assert.equal(String(r.subject.display || ''), '匿名-A');
    }

    // Provenance prefers not targeting Patient
    for (const e of on.bundle.entry) {
      const r = e.resource;
      if (!r || r.resourceType !== 'Provenance') continue;
      for (const t of r.target || []) {
        assert.notEqual(String(t.reference || ''), patientFullUrl);
      }
    }

    // patientDisplay alone (without includePatient) still has no identity
    const displayOnly = buildFhirExportBundle(analysis, {
      patientDisplay: 'should-not-wire',
    });
    assert.equal(displayOnly.counts.patients, 0);
    assert.ok(
      displayOnly.bundle.entry
        .map((e) => e.resource)
        .filter((r) => r && r.resourceType === 'Observation')
        .every((o) => !o.subject)
    );
    assert.ok(
      displayOnly.notes.some((n) => /patientDisplay ignored|includePatient is false/i.test(n))
    );

    assert.ok(on.validation && on.validation.ok, (on.validation.issues || []).join('; '));
    assert.equal(on.validation.resourceCounts.Patient, 1);

    // validation fails when subject does not resolve to Patient fullUrl
    const broken = JSON.parse(on.json);
    const obs = broken.entry
      .map((e) => e.resource)
      .find((r) => r && r.resourceType === 'Observation');
    assert.ok(obs);
    obs.subject = { reference: 'Patient/local-patient', display: 'x' };
    const bad = validateFhirExportBundle(broken);
    assert.equal(bad.ok, false);
    assert.ok(bad.issues.some((i) => /subject\.reference/i.test(i)));

    // helper builds standalone Patient (no default identifier)
    const standalone = buildLocalPatientResource({ display: 'Local user', gender: 'unknown' });
    assert.equal(standalone.resourceType, 'Patient');
    assert.equal(standalone.id, 'patient-local-1');
    assert.equal(standalone.name[0].text, 'Local user');
    assert.equal(standalone.gender, 'unknown');
    assert.ok(!standalone.identifier || !standalone.identifier.length);
    const strongPid = 'a1b2c3d4-e5f6-4789-a012-3456789abcde';
    const withPid = buildLocalPatientResource({
      display: 'X',
      persistentId: strongPid,
      birthYear: 1990,
    });
    assert.ok(withPid.identifier && withPid.identifier[0].value === strongPid);
    assert.equal(withPid.birthDate, '1990');
    // weak id must not be written
    const weak = buildLocalPatientResource({ display: 'Y', persistentId: '1' });
    assert.ok(!weak.identifier || !weak.identifier.length);
  });

  test('v1.56: day period date-precision; dateTime timezone rule; no shared patient id', () => {
    const { dayEffectivePeriod, isValidFhirDateTime } = lib;
    const p = dayEffectivePeriod('2026-07-10');
    assert.equal(p.start, '2026-07-10');
    assert.equal(p.end, '2026-07-10');
    assert.ok(isValidFhirDateTime('2026-07-10'));
    assert.ok(isValidFhirDateTime('2026-07-10T08:15:00+08:00'));
    assert.ok(isValidFhirDateTime('2026-07-10T00:00:00.000Z'));
    assert.ok(!isValidFhirDateTime('2026-07-10T00:00:00')); // time without TZ invalid

    const analysis = analyzeAll(parseHealthXml(SAMPLE_XML));
    // inject a steps day so we get effectivePeriod date-only
    analysis.stepsByDate = analysis.stepsByDate || {};
    analysis.stepsByDate['2026-07-02'] = 1000;
    if (analysis.data) {
      analysis.data.steps = analysis.data.steps || {};
      analysis.data.steps['2026-07-02'] = { watch: 1000, iphone: 0, max: 1000 };
    }
    // rebuild via analyzeAll for consistency
    const data = analysis.data;
    data.steps['2026-07-02'] = { watch: 1000, iphone: 0, max: 1000 };
    const a2 = analyzeAll(data);
    const result = buildFhirExportBundle(a2);
    const steps = result.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && String(r.id || '').startsWith('obs-steps-'));
    if (steps) {
      assert.ok(steps.effectivePeriod);
      assert.match(String(steps.effectivePeriod.start), /^\d{4}-\d{2}-\d{2}$/);
      assert.ok(!/T/.test(String(steps.effectivePeriod.start)));
    }
    // patient with fixed local-patient id fails validation
    const badPat = {
      resourceType: 'Bundle',
      type: 'collection',
      entry: [
        {
          fullUrl: 'urn:uuid:aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeee0099',
          resource: {
            resourceType: 'Patient',
            id: 'patient-local-1',
            identifier: [{ system: 'urn:health-analyzer:patient', value: 'local-patient' }],
            name: [{ text: 'x' }],
          },
        },
      ],
    };
    const badV = validateFhirExportBundle(badPat);
    assert.equal(badV.ok, false);
    assert.ok(badV.issues.some((i) => /local-patient|merge risk/i.test(i)));
  });

  test('v1.57–1.61: high-confidence Device; per-sample source; strip private ext', () => {
    assert.match(FHIR_EXPORT_PROFILE, /v1\.\d/);
    assert.equal(resolveObservationDeviceClass('spo2'), 'apple-watch');
    assert.equal(resolveObservationDeviceClass('restingHeartRate'), 'apple-watch');
    // BP / CGM without sample source → omit device
    assert.equal(resolveObservationDeviceClass('bloodPressure'), null);
    assert.equal(resolveObservationDeviceClass('glucose'), null);
    assert.equal(resolveObservationDeviceClass('glucose', { hasHaeImport: true }), null);
    assert.equal(resolveObservationDeviceClass('bodyWeight', { hasHaeImport: true }), null);
    // v1.61 per-sample sourceName
    assert.equal(classifySourceNameToDevice('iPhone'), 'iphone');
    assert.equal(classifySourceNameToDevice('John的Apple Watch'), 'apple-watch');
    assert.equal(classifySourceNameToDevice('hae'), null);
    assert.equal(classifySourceNameToDevice('Scale'), null);
    assert.equal(
      resolveObservationDeviceClass('bloodPressure', { sampleSource: 'iPhone' }),
      'iphone'
    );
    assert.equal(
      resolveObservationDeviceClass('glucose', { sampleSource: 'CGM Sensor' }),
      null
    );
    assert.equal(
      resolveObservationDeviceClass('glucose', { sampleSource: 'hae' }),
      null
    );
    assert.equal(
      resolveObservationDeviceClass('steps', { stepsDay: { watch: 1000, iphone: 0, max: 1000 } }),
      'apple-watch'
    );
    assert.equal(
      resolveObservationDeviceClass('steps', { stepsDay: { watch: 0, iphone: 500, max: 500 } }),
      'iphone'
    );
    // multi-source steps: omit (not apple-health Device)
    assert.equal(
      resolveObservationDeviceClass('steps', { stepsDay: { watch: 400, iphone: 600, max: 600 } }),
      null
    );
    assert.equal(
      resolveObservationDevice('steps', { stepsDay: { watch: 400, iphone: 600, max: 600 } })
        .confidence,
      'none'
    );

    const watchDev = buildLocalDeviceResource('apple-watch');
    assert.equal(watchDev.resourceType, 'Device');
    assert.equal(watchDev.id, 'device-apple-watch');
    assert.ok(
      Array.isArray(watchDev.extension) &&
        watchDev.extension.some(
          (x) =>
            x &&
            x.url === 'urn:health-analyzer:extension:device-confidence' &&
            x.valueCode === 'high'
        )
    );

    const analysis = analyzeAll(parseHealthXml(SAMPLE_XML));
    const on = buildFhirExportBundle(analysis, { includeProvenance: false });
    // Watch metrics still yield Device(s)
    assert.ok(on.counts.devices >= 1, 'expected at least one high-confidence Device');
    const devices = on.bundle.entry
      .map((e) => e.resource)
      .filter((r) => r && r.resourceType === 'Device');
    assert.ok(devices.every((d) => /device-apple-watch|device-iphone/.test(String(d.id || ''))));
    assert.ok(
      devices.every((d) => !/hae|apple-health/i.test(String(d.id || ''))),
      'must not emit HAE/aggregate Device'
    );

    const deviceFullUrls = new Set(
      on.bundle.entry
        .filter((e) => e.resource && e.resource.resourceType === 'Device')
        .map((e) => String(e.fullUrl || ''))
    );
    const obsList = on.bundle.entry
      .map((e) => e.resource)
      .filter((r) => r && r.resourceType === 'Observation');
    const withDevice = obsList.filter((o) => o.device && o.device.reference);
    assert.ok(withDevice.length > 0, 'expected some high-confidence Observation.device');
    for (const o of withDevice) {
      assert.ok(deviceFullUrls.has(String(o.device.reference)));
    }

    const spo2 = obsList.find((r) => String(r.id || '').startsWith('obs-spo2-'));
    if (spo2) {
      assert.ok(spo2.device);
      assert.match(String(spo2.device.display || ''), /Apple Watch/i);
    }
    // SAMPLE_XML BP sourceName=iPhone → high-confidence iPhone Device
    const bp = obsList.find((r) => String(r.id || '').startsWith('obs-bp-'));
    if (bp) {
      assert.ok(bp.device, 'BP with sourceName=iPhone should wire Device');
      assert.match(String(bp.device.display || ''), /iPhone/i);
      assert.ok(
        Array.isArray(bp.extension) &&
          bp.extension.some(
            (x) => x && /source-name/i.test(String(x.url || '')) && /iPhone/i.test(String(x.valueString || ''))
          ),
        'BP should retain source-name extension'
      );
    }
    // CGM sourceName=CGM Sensor → not a measurement Device class we support
    const glucose = obsList.find((r) => String(r.id || '').startsWith('obs-glucose-'));
    if (glucose) {
      assert.ok(!glucose.device, 'CGM Sensor must not invent Watch/iPhone device');
      assert.ok(
        Array.isArray(glucose.note) &&
          glucose.note.some((n) => n && /sourceName/i.test(String(n.text || '')))
      );
    }

    // parse retains source on weight
    const parsed = parseHealthXml(SAMPLE_XML);
    assert.ok(parsed.weight.some((w) => w.source && /Scale/i.test(w.source)));
    assert.ok(parsed.bloodPressure.some((b) => b.source && /iPhone/i.test(b.source)));
    assert.ok(parsed.cgm.some((c) => c.source && /CGM|Sensor|Fingerstick/i.test(c.source)));

    // HAE batch must not re-label CGM as HAE Device
    const withHae = buildFhirExportBundle(analysis, {
      includeProvenance: true,
      importBatches: [
        normalizeImportBatch({
          id: 'batch_hae_device_test',
          createdAt: new Date().toISOString(),
          source: 'hae',
          files: [{ name: 'metrics.json', bytes: 10 }],
          totalBytes: 10,
          stats: { totalAdded: 1, totalUpdated: 0, totalSkipped: 0 },
    ...
    const g2 = withHae.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && String(r.id || '').startsWith('obs-glucose-'));
    if (g2) assert.ok(!g2.device, 'HAE presence must not mark CGM Observation.device');
    assert.ok(
      !withHae.bundle.entry.some(
        (e) =>
          e.resource &&
          e.resource.resourceType === 'Device' &&
          /hae/i.test(String(e.resource.id || ''))
      ),
      'no device-hae-import resource'
    );

    // stripPrivateFhirExtensions removes project URNs for HL7 Base validation
    const stripped = stripPrivateFhirExtensions(on.bundle);
    const strippedObs = stripped.entry
      .map((e) => e.resource)
      .find((r) => r && r.resourceType === 'Observation' && String(r.id || '').startsWith('obs-bp-'));
    if (strippedObs) {
      assert.ok(
        !Array.isArray(strippedObs.extension) ||
          !strippedObs.extension.some((x) => x && String(x.url || '').startsWith('urn:health-analyzer:'))
      );
    }
    const strippedMetaTags = (stripped.meta && stripped.meta.tag) || [];
    assert.ok(
      !strippedMetaTags.some((t) => t && String(t.system || '').startsWith('urn:health-analyzer:'))
    );

    const broken = JSON.parse(on.json);
    const anyObs = broken.entry
      .map((e) => e.resource)
      .find((r) => r && r.resourceType === 'Observation' && r.device);
    assert.ok(anyObs);
    anyObs.device = { reference: 'Device/not-in-bundle', display: 'x' };
    const bad = validateFhirExportBundle(broken);
    assert.equal(bad.ok, false);
    assert.ok(bad.issues.some((i) => /device\.reference/i.test(i)));
  });

  test('v1.58–1.59: export tiers + exchange purposes + independent gate', () => {
    assert.match(FHIR_EXPORT_PROFILE, /v1\.\d/);
    assert.equal(normalizeFhirExportTier('external-exchange'), 'external-exchange');
    assert.equal(normalizeFhirExchangePurpose(null), 'anonymous-share');
    assert.equal(normalizeFhirExchangePurpose('personal-handoff'), 'personal-handoff');
    assert.ok(typeof FHIR_EXCHANGE_GATE_ENGINE === 'string' && /v[2-9]/.test(FHIR_EXCHANGE_GATE_ENGINE));

    const analysis = analyzeAll(parseHealthXml(SAMPLE_XML));

    const archive = buildFhirExportBundle(analysis, {
      exportTier: 'local-archive',
      includeProvenance: false,
    });
    assert.equal(archive.exportTier, 'local-archive');
    assert.equal(archive.exchangePurpose, null);
    assert.equal(archive.exchangeReady, true);

    // anonymous-share (default exchange): no Patient, gate ok
    const exchange = buildFhirExportBundle(analysis, {
      exportTier: 'external-exchange',
      exchangePurpose: 'anonymous-share',
      includePatient: true, // must be forced off
      includeProvenance: false,
    });
    assert.equal(exchange.exportTier, 'external-exchange');
    assert.equal(exchange.exchangePurpose, 'anonymous-share');
    assert.equal(exchange.counts.patients, 0);
    assert.ok(
      exchange.bundle.entry
        .map((e) => e.resource)
        .filter((r) => r && r.resourceType === 'Observation')
        .every((o) => !o.subject)
    );
    assert.ok(exchange.exchangeValidation && exchange.exchangeValidation.ok, (exchange.exchangeValidation && exchange.exchangeValidation.issues || []).join('; '));
    assert.equal(exchange.exchangeReady, true);
    const purposeTag = (exchange.bundle.meta.tag || []).find(
      (t) => t && t.system === 'urn:health-analyzer:exchange-purpose'
    );
    assert.ok(purposeTag && purposeTag.code === 'anonymous-share');

    // personal-handoff without persistent id → blocked
    const handoffFail = buildFhirExportBundle(analysis, {
      exportTier: 'external-exchange',
      exchangePurpose: 'personal-handoff',
      patientDisplay: 'Handoff-A',
      includeProvenance: false,
    });
    assert.equal(handoffFail.counts.patients, 1);
    assert.equal(handoffFail.exchangeReady, false);
    assert.ok(
      handoffFail.exchangeValidation &&
        handoffFail.exchangeValidation.issues.some((i) => /persistent|personal-handoff|strong/i.test(i))
    );

    // v1.63: weak id "1" rejected
    assert.ok(!isStrongPersistentPatientId('1'));
    assert.ok(!isStrongPersistentPatientId('test'));
    assert.ok(isStrongPersistentPatientId(newPersistentPatientId()));
    const handoffWeak = buildFhirExportBundle(analysis, {
      exportTier: 'external-exchange',
      exchangePurpose: 'personal-handoff',
      patientDisplay: 'Handoff-A',
    ...
    assert.equal(handoffWeak.exchangeReady, false);
    assert.ok(
      handoffWeak.notes.some((n) => /weak|UUID|pid_/i.test(n))
    );

    // personal-handoff with strong UUID → ok
    const handoffPid = 'b2c3d4e5-f6a7-4890-b123-456789abcdef';
    const handoffOk = buildFhirExportBundle(analysis, {
      exportTier: 'external-exchange',
      exchangePurpose: 'personal-handoff',
      patientDisplay: 'Handoff-A',
      patientPersistentId: handoffPid,
      includeProvenance: false,
    });
    assert.equal(handoffOk.exchangeReady, true, (handoffOk.exchangeValidation && handoffOk.exchangeValidation.issues || []).join('; '));
    assert.equal(handoffOk.counts.patients, 1);
    const patient = handoffOk.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && r.resourceType === 'Patient');
    assert.ok(
      patient.identifier &&
        patient.identifier.some((id) => id && id.value === handoffPid)
    );
    const obsWithSub = handoffOk.bundle.entry
      .map((e) => e.resource)
      .filter((r) => r && r.resourceType === 'Observation');
    assert.ok(obsWithSub.every((o) => o.subject && o.subject.reference));

    // category still required
    const stripped = JSON.parse(exchange.json);
    const o0 = stripped.entry.map((e) => e.resource).find((r) => r && r.resourceType === 'Observation');
    delete o0.category;
    const gate = validateFhirR4ExchangeGate(stripped, {
      exportTier: 'external-exchange',
      exchangePurpose: 'anonymous-share',
    });
    assert.equal(gate.ok, false);
    assert.ok(gate.issues.some((i) => /category/i.test(i)));
  });

  test('v1.62: anonymous-share scrub sourceName; gate blocks leaks', () => {
    const SECRET = '王小明的 iPhone';
    // XML with personalized Apple sourceName
    const xml = [
      '<HealthData>',
      `<Record type="HKQuantityTypeIdentifierBloodPressureSystolic" value="120" startDate="2026-07-10 08:00:00 +0800" sourceName="${SECRET}"/>`,
      `<Record type="HKQuantityTypeIdentifierBloodPressureDiastolic" value="80" startDate="2026-07-10 08:00:00 +0800" sourceName="${SECRET}"/>`,
      `<Record type="HKQuantityTypeIdentifierBloodGlucose" value="5.5" startDate="2026-07-10 09:00:00 +0800" sourceName="CGM Sensor"/>`,
      `<Record type="HKQuantityTypeIdentifierRestingHeartRate" value="55" startDate="2026-07-10 06:00:00 +0800" sourceName="Apple Watch"/>`,
      '</HealthData>',
    ].join('\n');
    const analysis = analyzeAll(parseHealthXml(xml));
    assert.ok(
      analysis.data.bloodPressure.some((b) => b.source && b.source.includes('王小明')),
      'parser must retain personalized sourceName for local archive'
    );

    // Local archive may still keep sourceName for personal review
    const archive = buildFhirExportBundle(analysis, {
      exportTier: 'local-archive',
      includeProvenance: false,
    });
    assert.ok(archive.json.includes('王小明') || archive.json.includes(SECRET));

    // Anonymous exchange must scrub
    const anon = buildFhirExportBundle(analysis, {
      exportTier: 'external-exchange',
      exchangePurpose: 'anonymous-share',
      includeProvenance: false,
    });
    assert.equal(anon.exchangePurpose, 'anonymous-share');
    assert.equal(anon.exchangeReady, true, (anon.exchangeValidation && anon.exchangeValidation.issues || []).join('; '));
    assert.ok(!anon.json.includes('王小明'), 'anonymous JSON must not contain personal token from sourceName');
    assert.ok(!anon.json.includes(SECRET), 'anonymous JSON must not contain full personalized sourceName');
    assert.ok(!anon.json.includes('sourceName:'), 'anonymous JSON must not contain sourceName notes');
    // Device class iPhone is OK (generic)
    const bp = anon.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && String(r.id || '').startsWith('obs-bp-'));
    if (bp) {
      assert.ok(bp.device, 'BP still wires generic iPhone Device class');
      assert.ok(
        !Array.isArray(bp.extension) ||
          !bp.extension.some((x) => x && x.url === EXT_SOURCE_NAME),
        'no source-name extension on anonymous BP'
      );
    }

    // Gate rejects re-injected leak
    const leaked = JSON.parse(anon.json);
    const obs = leaked.entry.map((e) => e.resource).find((r) => r && r.resourceType === 'Observation');
    assert.ok(obs);
    obs.extension = [{ url: EXT_SOURCE_NAME, valueString: SECRET }];
    const bad = validateFhirR4ExchangeGate(leaked, {
      exportTier: 'external-exchange',
      exchangePurpose: 'anonymous-share',
    });
    assert.equal(bad.ok, false);
    assert.ok(bad.issues.some((i) => /source-name|sourceName|re-identify/i.test(i)));

    obs.extension = [];
    obs.note = [{ text: `sourceName: ${SECRET}` }];
    const badNote = validateFhirR4ExchangeGate(leaked, {
      exportTier: 'external-exchange',
      exchangePurpose: 'anonymous-share',
    });
    assert.equal(badNote.ok, false);
    assert.ok(badNote.issues.some((i) => /sourceName note|re-identify/i.test(i)));
    ...
    const dirty = {
      ...
      entry: [
        {
          fullUrl: 'urn:uuid:aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeee0001',
          resource: {
            resourceType: 'Observation',
            id: 'x',
            status: 'final',
            code: { text: 't' },
            extension: [{ url: EXT_SOURCE_NAME, valueString: SECRET }],
            note: [{ text: `sourceName: ${SECRET}` }, { text: 'local Apple Health / HAE import' }],
          },
        },
      ],
    };
    const clean = sanitizeAnonymousFhirBundle(dirty);
    const cr = clean.entry[0].resource;
    assert.ok(!cr.extension || !cr.extension.length);
    assert.ok(Array.isArray(cr.note) && cr.note.length === 1);
    assert.ok(!JSON.stringify(clean).includes('王小明'));
  });

  test('v1.65: anonymous-share remaps personal batch ids; gate blocks leaks', () => {
    const SECRET_BATCH = 'batch_王小明_2026-07-30';
    assert.ok(isOpaqueAnonymousBatchId(opaqueImportBatchId(SECRET_BATCH)));
    assert.ok(!isOpaqueAnonymousBatchId(SECRET_BATCH));

    const analysis = analyzeAll(parseHealthXml(SAMPLE_XML));
    // Simulate domain linkage to the personal batch id
    analysis.domainSourceBatches = {
      bloodPressure: [SECRET_BATCH],
      cgm: [SECRET_BATCH],
    };
    const batch = normalizeImportBatch({
      id: SECRET_BATCH,
      createdAt: new Date().toISOString(),
      source: 'hae',
      files: [{ name: '王小明_health.json', bytes: 100 }],
      totalBytes: 100,
      stats: { totalAdded: 1, totalUpdated: 0, totalSkipped: 0 },
      ruleVersion: PROVENANCE_RULE_VERSION,
    });
    ...
    assert.equal(anon.exchangeReady, true, (anon.exchangeValidation && anon.exchangeValidation.issues || []).join('; '));
    assert.ok(!anon.json.includes('王小明'), 'anonymous JSON must not contain personal batch id token');
    assert.ok(!anon.json.includes(SECRET_BATCH), 'anonymous JSON must not contain full personal batch id');
    assert.ok(!anon.json.includes('王小明_health.json'), 'anonymous JSON must not contain personal file name');

    // Batch ids present only as opaque remaps
    const opaque = opaqueImportBatchId(SECRET_BATCH);
    assert.ok(anon.json.includes(opaque) || anon.json.includes('batch_anon_'), 'should retain opaque batch mapping');

    const obs = anon.bundle.entry
      .map((e) => e.resource)
      .find(
        (r) =>
          r &&
          r.resourceType === 'Observation' &&
          Array.isArray(r.extension) &&
          r.extension.some((x) => x && x.url === EXT_SOURCE_BATCH_IDS)
      );
    if (obs) {
      const ext = obs.extension.find((x) => x.url === EXT_SOURCE_BATCH_IDS);
      assert.ok(isOpaqueAnonymousBatchId(String(ext.valueString).split(',')[0]));
      assert.ok(!String(ext.valueString).includes('王小明'));
    }

    const prov = anon.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && r.resourceType === 'Provenance');
    if (prov && Array.isArray(prov.entity)) {
      for (const ent of prov.entity) {
        const v = ent.what && ent.what.identifier && ent.what.identifier.value;
        if (v) {
          assert.ok(isOpaqueAnonymousBatchId(v), `provenance entity id must be opaque: ${v}`);
        }
      }
    }

    // Gate blocks re-injected personal batch id
    const leaked = JSON.parse(anon.json);
    const anyObs = leaked.entry
      .map((e) => e.resource)
      .find((r) => r && r.resourceType === 'Observation');
    assert.ok(anyObs);
    anyObs.extension = [
      ...(anyObs.extension || []),
      { url: EXT_SOURCE_BATCH_IDS, valueString: SECRET_BATCH },
    ];
    const bad = validateFhirR4ExchangeGate(leaked, {
      exportTier: 'external-exchange',
      exchangePurpose: 'anonymous-share',
    ...
    assert.equal(bad.ok, false);
    assert.ok(bad.issues.some((i) => /batch_anon|source-batch-ids|opaque/i.test(i)));
  });
