...
<Record type="HKCategoryTypeIdentifierSleepAnalysis" value="HKCategoryValueSleepAnalysisAsleepCore" startDate="2026-07-01 23:00:00 +0800" sourceName="Apple Watch"/>
  ...
  test('v1.57: Device resources + Observation.device fullUrl wiring', () => {
    assert.match(FHIR_EXPORT_PROFILE, /v1\.\d/);
    ...
    assert.equal(resolveObservationDeviceClass('bloodPressure'), 'iphone');
    assert.equal(resolveObservationDeviceClass('glucose'), 'apple-health');
    assert.equal(resolveObservationDeviceClass('glucose', { hasHaeImport: true }), 'hae-import');
    assert.equal(
    ...
    assert.equal(watchDev.status, 'active');
    assert.ok(
      Array.isArray(watchDev.deviceName) &&
        watchDev.deviceName.some((n) => n && /Apple Watch/i.test(String(n.name || '')))
    );

    const analysis = analyzeAll(parseHealthXml(SAMPLE_XML));
    // default includeDevices=true
    const on = buildFhirExportBundle(analysis, { includeProvenance: false });
    assert.ok(on.counts.devices >= 1, 'expected at least one Device');
    const devices = on.bundle.entry
      .map((e) => e.resource)
      .filter((r) => r && r.resourceType === 'Device');
    assert.equal(devices.length, on.counts.devices);
    const deviceFullUrls = new Set(
      on.bundle.entry
        .filter((e) => e.resource && e.resource.resourceType === 'Device')
        .map((e) => String(e.fullUrl || ''))
    );
    assert.ok([...deviceFullUrls].every((u) => /^urn:uuid:/i.test(u)));

    const obsList = on.bundle.entry
      .map((e) => e.resource)
      .filter((r) => r && r.resourceType === 'Observation');
    assert.ok(obsList.length > 0);
    // Most observations should carry a resolvable device reference
    const withDevice = obsList.filter((o) => o.device && o.device.reference);
    assert.ok(withDevice.length > 0, 'expected Observation.device wiring');
    for (const o of withDevice) {
      assert.ok(
        deviceFullUrls.has(String(o.device.reference)),
        `${o.id} device.reference must match Device fullUrl`
      );
    }

    // Watch metrics → Apple Watch display
    ...
    assert.ok(on.validation && on.validation.ok, (on.validation.issues || []).join('; '));
    assert.ok((on.validation.resourceCounts.Device || 0) >= 1);

    // includeDevices false → no Device / no device field
    const off = buildFhirExportBundle(analysis, {
      includeDevices: false,
      includeProvenance: false,
    });
    assert.equal(off.counts.devices, 0);
    assert.ok(
      off.bundle.entry.every((e) => !e.resource || e.resource.resourceType !== 'Device')
    );
    assert.ok(
      off.bundle.entry
        .map((e) => e.resource)
    ...
    assert.ok(off.notes.some((n) => /includeDevices:\s*false/i.test(n)));

    // HAE batch → glucose uses HAE device class
    ...
    const glucose = withHae.bundle.entry
      .map((e) => e.resource)
      .find((r) => r && String(r.id || '').startsWith('obs-glucose-'));
    if (glucose) {
      assert.ok(glucose.device);
      assert.match(String(glucose.device.display || ''), /HAE|Health Auto Export/i);
    }
    assert.ok(
      withHae.bundle.entry.some(
        (e) =>
          ...
          String(e.resource.id || '') === 'device-hae-import'
      )
    );

    // Broken device.reference fails validation
    const broken = JSON.parse(on.json);
  ...
  test('v1.58: export tiers local-archive vs external-exchange + independent gate', () => {
    assert.match(FHIR_EXPORT_PROFILE, /v1\.8/);
    assert.equal(normalizeFhirExportTier('external-exchange'), 'external-exchange');
    assert.equal(normalizeFhirExportTier('archive'), 'local-archive');
    assert.equal(normalizeFhirExportTier(null), 'local-archive');
    assert.ok(typeof FHIR_EXCHANGE_GATE_ENGINE === 'string' && FHIR_EXCHANGE_GATE_ENGINE.length > 0);
    assert.ok(
      validateFhirR4ExchangeGate !== validateFhirExportBundle,
      'exchange gate must be a distinct function from project self-check'
    );
    ...
    assert.ok(archive.validation && archive.validation.ok);
    // default archive does not force exchangeValidation unless requested
    assert.ok(archive.exchangeValidation == null || archive.exchangeValidation.ok);

    const exchange = buildFhirExportBundle(analysis, {
      exportTier: 'external-exchange',
      includeProvenance: false,
      includeDevices: false, // should be forced on
    });
    assert.equal(exchange.exportTier, 'external-exchange');
    assert.ok(exchange.counts.devices >= 1, 'exchange forces devices');
    assert.ok(exchange.exchangeValidation, 'exchangeValidation present');
    assert.equal(exchange.exchangeValidation.engine, FHIR_EXCHANGE_GATE_ENGINE);
    assert.ok(
      exchange.exchangeValidation.ok,
      (exchange.exchangeValidation.issues || []).join('; ')
    );
    assert.equal(exchange.exchangeReady, true);
    assert.ok(exchange.validation && exchange.validation.ok);
    assert.ok(
      exchange.notes.some((n) => /external-exchange|exchange-gate/i.test(n))
    );
    const kindTag = (exchange.bundle.meta.tag || []).find(
      (t) => t && t.system === 'urn:health-analyzer:export-kind'
    );
    assert.ok(kindTag && kindTag.code === 'external-exchange');

    // category on all observations
    const obsList = exchange.bundle.entry
      .map((e) => e.resource)
      .filter((r) => r && r.resourceType === 'Observation');
    assert.ok(obsList.length > 0);
    for (const o of obsList) {
      assert.ok(Array.isArray(o.category) && o.category.length > 0, `${o.id} missing category`);
    }

    // independent gate fails when category stripped
    ...
    // force exchange validation on archive
    const forced = buildFhirExportBundle(analysis, {
      exportTier: 'local-archive',
      runExchangeValidation: true,
      includeProvenance: false,
    });
    assert.ok(forced.exchangeValidation);
    assert.ok(forced.exchangeValidation.ok);
    // still ready because tier is archive
    assert.equal(forced.exchangeReady, true);
  });
