...
<Record type="HKCategoryTypeIdentifierSleepAnalysis" value="HKCategoryValueSleepAnalysisAsleepCore" startDate="2026-07-01 23:00:00 +0800" sourceName="Apple Watch"/>
  ...
  test('v1.55: optional Patient pseudonym + subject fullUrl; default no identity', () => {
    ...
    const withPid = buildLocalPatientResource({
      display: 'X',
      persistentId: 'uuid-persistent-1',
      birthYear: 1990,
    });
    assert.ok(withPid.identifier && withPid.identifier[0].value === 'uuid-persistent-1');
  ...
  test('v1.58–1.59: export tiers + exchange purposes + independent gate', () => {
    ...
    assert.ok(typeof FHIR_EXCHANGE_GATE_ENGINE === 'string' && /v[23]/.test(FHIR_EXCHANGE_GATE_ENGINE));
    ...
    assert.ok(
      handoffFail.exchangeValidation &&
        handoffFail.exchangeValidation.issues.some((i) => /persistent identifier|personal-handoff/i.test(i))
    );

    // personal-handoff with random persistent id → ok
    const handoffOk = buildFhirExportBundle(analysis, {
      exportTier: 'external-exchange',
      exchangePurpose: 'personal-handoff',
      patientDisplay: 'Handoff-A',
      patientPersistentId: 'uuid-local-handoff-001',
      includeProvenance: false,
    });
    ...
    assert.ok(
      patient.identifier &&
        patient.identifier.some((id) => id && id.value === 'uuid-local-handoff-001')
    );
