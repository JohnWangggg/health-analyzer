...
<Record type="HKCategoryTypeIdentifierSleepAnalysis" value="HKCategoryValueSleepAnalysisAsleepCore" startDate="2026-07-01 23:00:00 +0800" sourceName="Apple Watch"/>
  ...
  test('解析活动/血氧/呼吸/VO2/腕温/夜间心率', () => {
    ...
    assert.equal(w.spo2Count, 2);
    // 0.975 → 97.5%，0.94 → 94%
    assert.ok(Math.abs(w.spo2Sum / w.spo2Count - 95.75) < 0.01);
    assert.ok(Math.abs(w.spo2Min - 94) < 0.01);
  ...
  test('analyzeAll / 提示词 / 洞察含 Watch 章节', () => {
    const analysis = analyzeAll(parseHealthXml(SAMPLE_XML));
    ...
    assert.ok(/Watch 活动|血氧|VO₂|夜间心率|呼吸|腕温/.test(titles));
