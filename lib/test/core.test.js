...
<Record type="HKCategoryTypeIdentifierSleepAnalysis" value="HKCategoryValueSleepAnalysisAsleepCore" startDate="2026-07-01 23:00:00 +0800" sourceName="Apple Watch"/>
  ...
  test('v1.57–1.59: high-confidence Device only; no HAE/aggregate as device', () => {
    assert.match(FHIR_EXPORT_PROFILE, /v1\.\d/);
    ...
    // BP / CGM: no per-record source → omit device
    ...
    // BP/glucose must NOT invent device
    const bp = obsList.find((r) => String(r.id || '').startsWith('obs-bp-'));
    if (bp) assert.ok(!bp.device, 'BP must not invent iPhone device without per-sample source');
    const glucose = obsList.find((r) => String(r.id || '').startsWith('obs-glucose-'));
    if (glucose) assert.ok(!glucose.device, 'CGM must not invent device');
    ...
    assert.ok(withHae.notes.some((n) => /HAE.*Provenance|not used as Observation\.device/i.test(n)));
